...
async function fixCustomerStatsIncludePaid() {
  try {
    // Get all merchants
    ...
    for (const merchant of merchants) {
      ...
      for (const customer of customers) {
        // Count unique visits from bookings (COMPLETED or PAID)
    ...
    // Special check for Lukas Nguyen
    console.log('\n=== Checking Lukas Nguyen specifically ===');
    const lukas = await prisma.customer.findFirst({
      where: { 
        firstName: 'Lukas',
        lastName: 'Nguyen'
      },
    ...
    if (lukas) {
      console.log(`Lukas: ${lukas.bookings.length} paid/completed bookings, ${lukas.orders.length} standalone orders`);
      console.log(`Current stats: ${lukas.visitCount} visits, $${lukas.totalSpent}`);
    }
