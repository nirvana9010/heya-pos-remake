...
test.describe("Staff Availability Checking", () => {
  ...
  test("should properly check staff availability and prevent double bookings", async ({
    page,
  }) => {
    ...
    await page.click(
      'button[class*="rounded-lg"][class*="border-2"]:first-child',
    );
    await page.click('button:has-text("Next")');

    // Now we should be back at datetime step with availability info
    await page.waitForSelector('label:has-text("Staff Member")', {
      timeout: 5000,
    });
    ...
    const availableStaffOptions = await page
      .locator('[role="option"]:has(.text-green-600)')
      .all();
    const unavailableStaffOptions = await page
      .locator('[role="option"]:has(.text-red-600)')
      .all();
    ...
    await page.fill(
      ...
      "Test Customer Availability 1",
    );
    await page.fill('input[placeholder="0400 123 456"]', "0400111111");
    await page.click('button:has-text("Next")');

    // Confirm booking
    await page.waitForSelector('h4:has-text("Booking Summary")', {
      timeout: 5000,
    });

    // Verify staff shown in summary
    const staffSummaryText = await page
      .locator('span:has-text("Staff:")')
      .locator("..")
    ...
    await page.waitForSelector(
      '.toast-success, [role="status"]:has-text("successfully"), div:has-text("Booking created")',
      {
        timeout: 10000,
      },
    );
    ...
    console.log(
      `✓ First booking created for ${selectedStaffName} at ${timeStr}\n`,
    );
    ...
    const availabilityMessage = await page
      .locator('div:has(.w-4.h-4):has-text("available")')
      .textContent()
      .catch(() => null);
    ...
    const staffOptionWithConflict = await page
      .locator(`[role="option"]:has-text("${selectedStaffName}")`)
      .first();
    if (await staffOptionWithConflict.isVisible()) {
      const isDisabled =
        await staffOptionWithConflict.getAttribute("aria-disabled");
      const hasConflictIndicator = await staffOptionWithConflict
        .locator(".text-red-600")
        .isVisible()
        .catch(() => false);

      console.log(
        `${selectedStaffName} is now shown as:`,
        isDisabled === "true" || hasConflictIndicator
          ? "UNAVAILABLE ✓"
          : "Still available ✗",
      );

      if (hasConflictIndicator) {
        const conflictReason = await staffOptionWithConflict
          .locator(".text-red-600")
          .textContent();
    ...
    const nextAvailableOption = await page
      .locator('[role="option"]:has-text("Next Available")')
      .first();
    ...
    const nextAvailableStaffText = await page
      .locator('span:has-text("Staff:")')
      .locator("..")
      .locator("span.font-medium")
      .textContent();
    ...
    if (nextAvailableStaffText?.includes(selectedStaffName)) {
      console.log(
        "❌ ERROR: Next Available assigned the same busy staff member!",
      );
    } else {
    ...
    try {
      ...
      console.log(
        "✓ Second booking created successfully with different staff\n",
      );
    } catch (error) {
      // Check for error message
      const errorVisible = await page
        .locator('.toast-error, [role="alert"], div:has-text("Error")')
        .isVisible();
      if (errorVisible) {
    ...
    const noStaffMessage = await page
      .locator("text=/no staff available/i")
      .isVisible()
      .catch(() => false);
    if (noStaffMessage) {
  ...
  test("should update availability when changing date/time", async ({
    page,
  }) => {
    ...
    await page.click('button[role="combobox"]');
    const initialAvailable = await page
      .locator('[role="option"]:has(.text-green-600)')
      .count();
    ...
    // Check again
    await page.click('button[role="combobox"]');
    const laterAvailable = await page
      .locator('[role="option"]:has(.text-green-600)')
      .count();
