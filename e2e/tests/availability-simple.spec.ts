...
test.describe("Staff Availability - Simple Test", () => {
  test("verify availability checking works", async ({ page }) => {
    ...
    const hasTimeInput = await page
      ...
      .isVisible()
      .catch(() => false);
    const hasStaffSelect = await page
      .locator('select, [role="combobox"], button:has-text("Select staff")')
      .isVisible()
      .catch(() => false);
    ...
    if (!hasDateInput && !hasTimeInput) {
      ...
      if (bookingButton) {
        ...
        const modalVisible = await page
          .locator('[role="dialog"], .modal, .slide-out, aside')
          .isVisible()
          .catch(() => false);
        ...
        if (modalVisible) {
          ...
          const modalDateInput = await page
            .locator(
              '[role="dialog"] input[type="date"], .modal input[type="date"], aside input[type="date"]',
            )
            .isVisible()
            .catch(() => false);
          const modalStaffSelect = await page
            .locator(
              '[role="dialog"] [role="combobox"], .modal [role="combobox"], aside [role="combobox"]',
            )
            .isVisible()
            .catch(() => false);
          ...
          if (modalDateInput && modalStaffSelect) {
            ...
            const availableIndicators = await page
              .locator(
                '.text-green-600, svg.text-green-600, [aria-label*="available"]',
              )
              .count();
            const unavailableIndicators = await page
              .locator(
                '.text-red-600, svg.text-red-600, [aria-disabled="true"]',
              )
              .count();
            ...
            const nextAvailable = await page
              .locator("text=/next available/i")
              .isVisible();
            console.log('Has "Next Available" option:', nextAvailable);

            // Check for availability message
            const availabilityMsg = await page
              .locator("text=/available|unavailable|busy/i")
              .first()
              .textContent()
              .catch(() => null);
            if (availabilityMsg) {
