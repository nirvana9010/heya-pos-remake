...
test.describe("Full Availability Checking Test", () => {
  test("should demonstrate real availability checking", async ({ page }) => {
    ...
    const newBookingBtn = page
      .locator(
        'button:has-text("+ New Booking"), button:has-text("New Booking")',
      )
      .first();
    ...
    const slideOut = page
      .locator('aside, [role="dialog"], .slide-out-panel')
      .first();
    ...
    if (!isSlideOutVisible) {
      ...
      const anyPanel = await page
        .locator('[class*="fixed"][class*="right"]')
        .first()
        .isVisible();
    ...
    const availableCount = await page
      .locator(
        '[role="option"] svg.text-green-600, [role="option"] .text-green-600',
      )
      .count();
    ...
    const unavailableCount = await page
      .locator(
        '[role="option"][aria-disabled="true"], [role="option"] .text-red-600',
      )
      .count();
    console.log("Staff marked as unavailable:", unavailableCount);

    // Check Next Available option
    const nextAvailableOption = page
      .locator('[role="option"]:has-text("Next Available")')
      .first();
    const hasNextAvailable = await nextAvailableOption
      .isVisible()
      .catch(() => false);
    ...
    await page.fill(
      'input[placeholder*="John"], input[placeholder*="Name"]',
      "Test Availability 1",
    );
    await page.fill(
      'input[placeholder*="0400"], input[placeholder*="Phone"]',
      "0400111111",
    );
    ...
    const confirmationVisible = await page
      .locator('h4:has-text("Booking Summary")')
      .isVisible()
      .catch(() => false);
    ...
    await page
      .click('button:has-text("+ New Booking"), button:has-text("New Booking")')
      .first();
    ...
    const nowUnavailable = await page
      .locator(
        '[role="option"] .text-red-600, [role="option"][aria-disabled="true"]',
      )
      .count();
    console.log("Staff now marked as unavailable:", nowUnavailable);

    // Check for conflict messages
    const conflictMessages = await page
      .locator(
        '.text-red-600:has-text("Booked"), .text-red-600:has-text("busy")',
      )
      .count();
    console.log("Conflict messages shown:", conflictMessages);

    // Check if Next Available shows reduced count
    const nextAvailableUpdated = await page
      .locator('[role="option"]:has-text("Next Available")')
      .first()
      .textContent()
      .catch(() => "");
