  ...
  private async checkESLint() {
    ...
    try {
      ...
      if (warningMatch && parseInt(warningMatch[1]) > 10) {
        this.warnings.push(
          `ESLint found ${warningMatch[1]} warnings (max 10 allowed)`,
        );
      }
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (uniqueVersions.size > 1) {
          this.warnings.push(
            "Multiple React versions detected - this may cause issues",
          );
        }
      } catch {
  ...
  private async checkBundleSize() {
    ...
    if (existsSync(nextConfigPath)) {
      ...
      if (!content.includes("swcMinify")) {
        this.warnings.push(
          "SWC minification not enabled - bundles may be larger than necessary",
        );
      }

      if (!content.includes("optimizePackageImports")) {
        this.warnings.push(
          "Package imports not optimized - consider enabling optimizePackageImports",
        );
      }
