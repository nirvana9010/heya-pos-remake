...
<div>
  ...
  <Input id="name" placeholder="Placeholder text" className="w-full" />
</div>
```
...
<div className="space-y-8">
  <section className="space-y-4">{/* Section content */}</section>
</div>
```
...
<div className="fixed inset-y-0 right-0 flex max-w-full pl-10 z-50">
  <div className="w-screen max-w-md">
    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
      {/* Header */}
      <div className="px-4 py-6 sm:px-6 border-b">{/* Content */}</div>

      {/* Body */}
      <div className="flex-1 px-4 py-6 sm:px-6">{/* Content */}</div>

      {/* Footer */}
      <div className="px-4 py-4 sm:px-6 border-t">{/* Actions */}</div>
    </div>
  </div>
</div>
```
...
```tsx
<div className="text-sm text-red-600 mt-1">Error message here</div>
```
