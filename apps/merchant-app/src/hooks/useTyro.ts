...
export const useTyro = () => {
  ...
  const getIClientWithUI = useCallback(() => {
    if (!window.TYRO) {
      throw new Error(
        "Tyro SDK not loaded. Please ensure iclient-with-ui-v1.js is included.",
      );
    }
        ...
        iClient.pairTerminal(merchantId, terminalId, (response: any) => {
          ...
          if (response.status === "success") {
            ...
            sessionStorage.setItem(
              TYRO_STORAGE_KEYS.CLIENT,
              JSON.stringify(iClient),
            );
  ...
  const purchase = useCallback(
    (amount: number, callbacks?: TyroTransactionCallbacks) => {
      try {
        ...
        iClient.initiateRefund(
          { amount: amountInCents, integratedReceipt: true },
          {
            ...
            transactionCompleteCallback: (
              response: TyroTransactionResponse,
            ) => {
