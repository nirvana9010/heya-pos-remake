...
export async function GET(request: NextRequest) {
  try {
    ...
    const scenarios = [
      {
        ...
        description:
          "Customer books a service, receives confirmation email/SMS",
        steps: [
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || "Failed to load test scenarios",
      },
      { status: 500 },
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/test-notifications/test-scenarios`,
      {
        method: "POST",
        headers: {
