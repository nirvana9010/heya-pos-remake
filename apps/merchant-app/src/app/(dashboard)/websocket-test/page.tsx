...
export default function WebSocketTestPage() {
  const [events, setEvents] = useState<
    Array<{ time: string; type: string; data: any }>
  >([]);
  ...
  const { connectionStatus, lastNotification, isRealtime, reconnect, emit } =
    useRobustWebSocket({
      debug: true,
    ...
    setEvents((prev) =>
      [
        ...prev,
        {
          ...
          data: {
            action:
              "Simulated token expiry - set invalid token and forced reconnect",
          },
        },
      ].slice(-20),
    );
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: "0.875rem", fontFamily: "monospace" }}>
              {lastNotification
                ? new Date(lastNotification).toLocaleTimeString()
                : "None"}
            </p>
          </div>
        ...
        >
          ...
          {connectionStatus.reconnectAttempt && (
            ...
            >
              Reconnection attempt {connectionStatus.reconnectAttempt} of{" "}
      ...
      >
        ...
        >
          ...
          >
            <strong>Proactive Refresh:</strong> Token will automatically refresh
            1 hour before expiry
          </p>
          <p style={{ fontSize: "0.875rem", color: "#1e40af" }}>
            <strong>Reactive Refresh:</strong> If auth fails, token will refresh
          ...
          {events.length === 0 ? (
            ...
            [...events].reverse().map((event, index) => (
              <div
                ...
                style={{
                  borderBottom:
                    index < events.length - 1 ? "1px solid #f3f4f6" : "none",
