...
export default function EditBookingPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (
        typeof bookingData.startTime === "string" &&
        bookingData.startTime.includes("T")
      ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (booking.startTime) {
        ...
        const newDateInMerchantTZ = new Date(
          year,
          month - 1,
          day,
          hours,
        ...
        const newDateUTC = zonedTimeToUtc(
          newDateInMerchantTZ,
          "Australia/Sydney",
        );
      ...
      if (timeChanged || staffChanged || servicesChanged) {
        ...
        const [year, month, day] = formData.date.split("-").map(Number);
        const [hours, minutes] = formData.startTime.split(":").map(Number);
        const dateInMerchantTZ = new Date(
          year,
          month - 1,
          day,
          hours,
    ...
    <div className="container mx-auto p-6 space-y-6">
                                  ...
                                  {staff
                                    .filter(
                                      (s) =>
                                        !s.services ||
                                        s.services.length === 0 ||
                                        s.services.includes(service.serviceId),
                                    )
                                    .map((member) => (
                                      <SelectItem
                                        key={member.id}
      ...
      <ServiceSelectionSlideout
        ...
        onSelectService={(selectedService) => {
          ...
          if (
            formData.services.some((s) => s.serviceId === selectedService.id)
          ) {
