...
export default function BookingsManager() {
  ...
  const [dateFilter, setDateFilter] = useState<"upcoming" | "all" | "past">(
    "upcoming",
  );
  ...
  const parseBookingUpdateError = (response: any) => {
    ...
    if (response.success === false) {
      return (
        response.message || response.errorMessage || "Failed to update booking"
      );
    }
  ...
  const handleCheckIn = async (bookingId: string) => {
    try {
      ...
      setBookings((prevBookings) =>
        prevBookings.map((b) =>
          b.id === bookingId ? { ...b, status: "in-progress" } : b,
        ),
      );
  ...
  const handleBulkSendReminders = async () => {
    ...
    try {
      const selectedBookingsList = bookings.filter((b) =>
  ...
  const handleMarkPaid = async (bookingId: string, amount: number) => {
    ...
    try {
      ...
      const response = await apiClient.post(
        `/v2/bookings/${bookingId}/mark-paid`,
        {
          paymentMethod: "CASH",
          amount: amount,
        },
  ...
  const stats = useMemo(() => {
    ...
    if (futureBookings.length === 0) {
      ...
      const firstBookingTime = new Date(
        firstBooking.startTime || firstBooking.date,
      );
      const minutesUntilFirst = Math.floor(
        (firstBookingTime.getTime() - now.getTime()) / 60000,
      );
      ...
      if (minutesUntilFirst >= 30) {
        ...
        for (let i = 0; i < futureBookings.length - 1; i++) {
          const currentEnd = new Date(
            futureBookings[i].endTime ||
              new Date(futureBookings[i].startTime).getTime() +
                (futureBookings[i].duration || 60) * 60000,
          );
          const nextStart = new Date(
            futureBookings[i + 1].startTime || futureBookings[i + 1].date,
          );
        ...
        if (!foundSlot) {
          ...
          const lastEndTime = new Date(
            lastBooking.endTime ||
              new Date(lastBooking.startTime).getTime() +
                (lastBooking.duration || 60) * 60000,
          );
    ...
    const pendingBookings = bookings.filter((b) => {
      const bookingDate = new Date(b.startTime || b.date);
      return b.status?.toLowerCase() === "pending" && bookingDate >= now; // Only future pending bookings
    });
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {stats.nextAvailableTime}
                  </p>
                  <p className="text-sm text-blue-600">
                    {stats.nextAvailableStaff}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                >
                  <AlertCircle
                    className={`h-6 w-6 ${stats.pendingCount > 3 ? "text-yellow-600" : "text-green-600"}`}
                  />
                </div>
              </div>
            </CardContent>
                          ...
                          <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-4 mb-2">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {isStartingVerySoon && (
                                    <Badge
                                      variant="outline"
                                      className="border-orange-500 text-orange-600 animate-pulse"
                                    >
                            ...
                            <div onClick={(e) => e.stopPropagation()}>
                              <BookingActions
                                ...
                                isPaymentProcessing={processingPayments.has(
                                  booking.id,
                                )}
                                isProcessingPayment={processingOrders.has(
                                  booking.id,
                                )}
                                  ...
                                  try {
                                    ...
                                    switch (status) {
                                      case "in-progress":
                                        ...
                                        break;
                                      case "completed":
                                        await apiClient.completeBooking(
                                          bookingId,
                                        );
                                        break;
                                      case "cancelled":
        ...
        {selectedBookingForDetails && (
          <BookingDetailsSlideOut
            ...
            booking={{
              ...
              startTime: selectedBookingForDetails.startTime
                ? new Date(selectedBookingForDetails.startTime)
                : new Date(),
              endTime: selectedBookingForDetails.endTime
                ? new Date(selectedBookingForDetails.endTime)
                : new Date(),
              duration: selectedBookingForDetails.duration || 60,
              status: selectedBookingForDetails.status || "pending",
              isPaid:
                selectedBookingForDetails.paymentStatus === "PAID" ||
                selectedBookingForDetails.paymentStatus === "paid",
              totalPrice:
              ...
              customerRequestedStaff: Boolean(
                selectedBookingForDetails.customerRequestedStaff,
              ),
            }}
            staff={staff}
            services={services}
            ...
            onSave={async (updatedBooking) => {
              try {
                ...
                const totalPrice =
                  updatedBooking.services?.reduce((sum: number, s: any) => {
                ...
                const serviceName =
                  updatedBooking.services?.length > 1
                    ? updatedBooking.services
                        .map((s: any) => s.name)
                        .join(" + ")
                    : updatedBooking.services?.[0]?.name ||
                ...
                const mappedServices = updatedBooking.services?.map(
                  (s: any) => ({
                ...
                const updatePromise = apiClient.updateBooking(
                ...
                try {
                  ...
                  const detectedErrorMessage =
                    parseBookingUpdateError(apiResponse);
                  if (detectedErrorMessage) {
                    const error = new Error(
                      detectedErrorMessage || "Update failed",
                    );
            ...
            onStatusChange={async (bookingId, status) => {
              try {
                ...
                setBookings((prevBookings) =>
                  prevBookings.map((b) =>
                    b.id === bookingId ? { ...b, status: localStatus } : b,
                  ),
                );
                    ...
                    prevBookings.map((b) =>
                      b.id === bookingId
                        ? {
                            ...
                            isPaid: true,
                            paidAmount:
                              paidAmount || b.totalAmount || b.price || 0,
                            paymentStatus: "PAID",
                          }
                        : b,
                    ),
