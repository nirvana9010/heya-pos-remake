...
export const BookingProvider: React.FC<BookingProviderProps> = ({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const overrideStart = format(
        addDays(startOfWeek(new Date()), -14),
        "yyyy-MM-dd",
      );
      const overrideEnd = format(
        addDays(endOfWeek(new Date()), 35),
        "yyyy-MM-dd",
      );
      ...
      await Promise.all(
        activeStaff.map(async (member: any) => {
          try {
            ...
            console.error(
              "[BookingContext] Failed to load overrides for staff",
              member.id,
              error,
            );
