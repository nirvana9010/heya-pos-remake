...
export function NotificationsProvider({
  ...
  const [optimisticNotifications, setOptimisticNotifications] = React.useState<
  ...
  const convertNotification = useCallback(
    (apiNotification: MerchantNotification): Notification => ({
  ...
  const notifications = useMemo(() => {
    ...
    return [...optimisticConverted, ...converted].sort(
  ...
  const shownBrowserNotificationsRef = React.useRef<Set<string>>(
  ...
  const processNotifications = React.useCallback(() => {
    ...
    if (newBookingNotifications.length > 0) {
      ...
      newBookingNotifications.forEach((notification) => {
        console.log(
          "[NotificationContext] Broadcasting booking-updated event for:",
          {
  ...
  useEffect(() => {
    if (
      ...
      queryResult.dataUpdatedAt !== lastDataTimestampRef.current
    ) {
