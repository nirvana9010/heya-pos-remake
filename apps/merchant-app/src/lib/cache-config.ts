...
export function getCacheConfig(endpoint: string): CacheConfig {
  ...
  const endpointType = Object.keys(cacheConfigs).find(
    (key) => endpoint.includes(key) || endpoint.includes(key.replace("-", "/")),
  );
...
export function generateCacheKey(endpoint: string, params?: any): string {
  ...
  const merchantStr =
    typeof window !== "undefined" ? localStorage.getItem("merchant") : null;
  if (merchantStr) {
    try {
