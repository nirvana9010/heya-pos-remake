...
export function PaymentDialog({
  ...
  const handleLoyaltyRedemption = useCallback(
  ...
  const handlePayment = async () => {
    ...
    if (order?.isCached && !order?.id) {
      toast({
        title: "Please wait",
        description:
          "Order is still being created. Please try again in a moment.",
      });
      return;
    }
    ...
    if (
      ...
      isTyroEnabled &&
      merchant?.settings?.tyroTerminalId
    ) {
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    if (
                      isPercentage ||
                      loyaltyDiscount.description.includes("Free Service")
                    ) {
                      await apiClient.loyalty.redeemVisit(
                        customer.id,
                        order.id,
                      );
                    } else {
                      const pointsMatch =
                        loyaltyDiscount.description.match(/\((\d+) points\)/);
                      if (pointsMatch) {
                        const points = parseInt(pointsMatch[1]);
                        await apiClient.loyalty.redeemPoints(
                          customer.id,
                          points,
                          order.id,
                        );
                    ...
                    console.error(
                      "Failed to redeem loyalty after Tyro payment:",
                      error,
                    );
                  }
                }
    ...
    try {
      ...
      if (order?.state === "DRAFT" && !orderAdjustment.amount) {
        console.log(
          "[PaymentDialog] Locking order before payment (no adjustments)",
        );
      ...
      const adjustmentAmount = calculateAdjustmentAmount();
      const hasAdjustmentAlreadyApplied = order?.modifiers?.some(
        (mod: any) =>
      ...
      if (adjustmentAmount !== 0 && order?.id && !hasAdjustmentAlreadyApplied) {
        try {
          ...
          if (order.state !== "DRAFT" && order.state !== "LOCKED") {
            console.warn(
              "[PaymentDialog] Order is in state",
              order.state,
              "- may not accept modifiers",
            );
          }
          ...
          const updatedOrder = await apiClient.addOrderModifier(
            order.id,
            modifier,
          );
          ...
          if (updatedOrder.state === "DRAFT") {
            console.log(
              "[PaymentDialog] Locking order after applying modifier",
            );
            const lockedOrder = await apiClient.updateOrderState(
              updatedOrder.id,
              "LOCKED",
            );
            order = lockedOrder;
          ...
          const errorDetails = {
            message:
              ...
              modifierError?.response?.data?.message ||
              "Unknown error",
          ...
          console.error(
            "[PaymentDialog] Failed to apply order modifier:",
            errorDetails,
          );
          ...
          toast({
            ...
            description:
              modifierError?.message ||
              "The discount/surcharge could not be applied. Please try again.",
            variant: "destructive",
          });
      ...
      console.log(
        "[PaymentDialog] Payment successful, calling onPaymentComplete:",
        {
          id: successOrder.id,
      ...
      if (loyaltyDiscount.amount > 0) {
        if (!customer || !customer.id) {
          ...
          toast({
            ...
            description:
              "Customer information is missing. Payment was successful but loyalty discount was not applied.",
            variant: "warning",
          });
        } else if (
          isWalkInCustomer(customer.id) ||
          ...
          console.error(
            "Loyalty redemption failed: Walk-in customers cannot use loyalty",
            {
              customer,
              loyaltyDiscount,
              orderId: order.id,
          ...
          try {
            ...
            const errorMessage =
              error?.message || "Failed to redeem loyalty reward";
            toast({
              title: "Loyalty Redemption Failed",
      ...
      const errorMessage =
        error?.message || error?.response?.data?.message || "Payment failed";
      const isStateError =
        errorMessage.includes("not ready for payment") ||
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            ...
                            {hasAdjustment && (
                              <span
                                className={`ml-2 ${isDiscount ? "text-green-600" : "text-red-600"}`}
                              >
                                ({isDiscount ? "-" : "+"}$
                        ...
                        <div className="text-right">
                          {hasAdjustment ? (
                            <div>
                              ...
                              <div
                                className={`text-sm font-medium ${isDiscount ? "text-green-600" : "text-red-600"}`}
                              >
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="flex bg-gray-100 rounded-md p-1">
                      <Button
                        size="sm"
                        variant={
                          orderAdjustment.isPercentage ? "ghost" : "default"
                        }
                        onClick={() =>
                      ...
                      <Button
                        size="sm"
                        variant={
                          orderAdjustment.isPercentage ? "default" : "ghost"
                        }
                        onClick={() =>
                          setOrderAdjustment((prev) => ({
                            ...prev,
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            {!order || (order.isLoading && !order.isCached) ? (
              ...
              (() => {
                ...
                const totalAfterModifiers = existingModifiers.reduce(
                  (total, modifier) => {
                ...
                const totalWithAdjustment =
                  totalAfterModifiers + adjustmentAmount;
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between font-bold text-lg">
                      ...
                      <span>
                        ...
                        {Math.max(
                          0,
                          totalWithLoyalty - (order?.paidAmount || 0),
                        ).toFixed(2)}
                      </span>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select
                        value={sp.method}
                        onValueChange={(v) =>
                          updateSplitPayment(index, "method", v)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
              ...
              {(() => {
                ...
                return (
                  splitRemaining !== 0 && (
                    <div
                      className={`text-center font-medium ${splitRemaining > 0 ? "text-red-600" : "text-green-600"}`}
                    >
                      {splitRemaining > 0 ? "Remaining" : "Overpayment"}: $
            ...
            <div className="border-t pt-4 space-y-3">
              ...
              {tipAmount > 0 && (
                <div className="text-sm text-gray-600">
                  Tip: ${tipAmount.toFixed(2)} | Total with tip: $
