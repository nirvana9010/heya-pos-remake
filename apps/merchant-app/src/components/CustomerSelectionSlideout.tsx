...
export const CustomerSelectionSlideout: React.FC<
  CustomerSelectionSlideoutProps
> = ({
  isOpen,
  onClose,
  onSelectCustomer,
  ...
  useEffect(() => {
    ...
    const focusTarget = showCreateForm
      ...
      try {
        ...
        const loyaltyPromises = customerData.map(async (customer: Customer) => {
          try {
            ...
            console.error(
              `Failed to check loyalty for customer ${customer.id}:`,
              error,
            );
  ...
  const handleCreateCustomer = async () => {
    ...
    try {
      ...
      const newCustomer =
        await apiClient.customers.createCustomer(customerData);
