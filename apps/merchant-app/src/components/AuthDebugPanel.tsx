...
export function AuthDebugPanel() {
  const { isAuthenticated, isLoading, user, merchant, tokenExpiresAt, error } =
    useAuth();
        ...
        <div className="fixed bottom-20 right-4 w-96 max-h-[600px] overflow-auto bg-white border rounded-lg shadow-2xl p-4 z-[9998]">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`inline-block w-2 h-2 rounded-full ${authData.isAuthenticated ? "bg-green-500" : "bg-red-500"}`}
              />
            ...
            <div className="flex items-center gap-2">
              <span
                className={`inline-block w-2 h-2 rounded-full ${authData.isLoading ? "bg-yellow-500" : "bg-gray-500"}`}
              />
            ...
            <div className="border-t pt-2 mt-2">
              <p>
                <strong>User:</strong> {authData.userName} ({authData.userEmail}
                )
              </p>
              <p>
                <strong>Merchant:</strong> {authData.merchantName} (ID:{" "}
                {authData.merchantId})
              </p>
            </div>
            ...
            <div className="border-t pt-2 mt-2">
              <p
                className={
                  authData.hasAccessToken ? "text-green-600" : "text-red-600"
                }
              >
              ...
              <p
                className={
                  authData.hasRefreshToken ? "text-green-600" : "text-red-600"
                }
              >
              ...
              <p
                className={
                  authData.hasCookie ? "text-green-600" : "text-red-600"
                }
              >
            ...
            {authData.tokenExpiry && (
              <div className="border-t pt-2 mt-2">
                <p>
                  ...
                  {new Date(authData.tokenExpiry).toLocaleString()}
                </p>
                <p
                  className={
                    authData.tokenExpired ? "text-red-600" : "text-green-600"
                  }
                >
            ...
            {authData.authError && (
              <div className="border-t pt-2 mt-2 text-red-600">
                <p>
                  <strong>Error:</strong> {authData.authError}
                </p>
              </div>
            )}
            ...
            <div className="border-t pt-2 mt-2">
              <p>
                <strong>Current Path:</strong> {authData.currentPath}
              </p>
            </div>
