...
function autoDetectField(header: string): string | null {
  ...
  if (lower.includes("full name") || lower === "fullname" || lower === "name")
    return "Full Name";
  ...
  if (lower.includes("marketing") || lower.includes("opt in"))
    return "Accepts Marketing";
  if (lower.includes("blocked") || lower.includes("blacklist"))
    return "Blocked";
  if (lower.includes("block reason")) return "Block Reason";
  if (lower.includes("referral") || lower.includes("source"))
...
export function CustomerColumnMappingDialog({
  ...
  const validateMappings = () => {
    ...
    if (missingRequired.length > 0) {
      setError(
        `Required fields not mapped: ${missingRequired.map((field) => FIELD_LABELS[field]).join(", ")}`,
      );
      return false;
    }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Match your CSV columns to the customer fields. At minimum, map the
            first name column so we can create customer records correctly.
          </DialogDescription>
        </DialogHeader>
              ...
              <TableBody>
                {mappings.map((mapping, index) => {
                  const availableFields = new Set(
                    getAvailableFields(mapping.field),
                  );
