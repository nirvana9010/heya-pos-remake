...
const mapServicesForCalendar = (
  services?: BookingServiceSummary[],
): BookingServiceSummary[] | undefined => {
...
const baseCalendarBooking = (booking: ApiBooking): Booking => {
  ...
  const serviceName =
    ...
    "Service";
  const servicePrice =
    booking.totalAmount ?? booking.price ?? primaryService?.price ?? 0;
  const completedAt =
    booking.completedAt ??
...
export const mapNormalizedBookingToCalendar = (booking: ApiBooking): Booking =>
  baseCalendarBooking(booking);
...
export const mapCustomerToCalendar = (customer: ApiCustomer): Customer => ({
  ...
  tags: Array.isArray((customer as any).tags)
...
export const mapServiceToCalendar = (service: ApiService): Service => ({
  ...
  categoryColor:
    ...
    (service as any).categoryColorHex,
...
export const mapStaffToCalendar = (staff: ApiStaff): Staff => {
  ...
  return {
    id: staff.id,
    name:
      formatName(staff.firstName ?? undefined, staff.lastName ?? undefined) ||
      staff.firstName ||
      "Staff",
    email: staff.email,
    role: staff.role,
    ...
    calendarColor:
      (staff as any).calendarColor && (staff as any).calendarColor !== "auto"
