...
import {
  toMerchantTime,
  formatInMerchantTime,
  formatMerchantDateTimeISO,
} from "@/lib/date-utils";
import {
  startOfDay,
  endOfDay,
...
const isBlocksEnabled = (merchant: any): boolean => {
  if (
    merchant?.settings &&
    merchant.settings.enableCalendarBlocks !== undefined
  ) {
...
export function useCalendarData() {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("calendar-fetch-bookings", {
    ...
    try {
      ...
      const transformedBookings = response.map((booking: any) => {
        ...
        const durationFromTimestamps =
          ...
          !Number.isNaN(endTimeRaw.getTime())
            ? Math.max(
                0,
                Math.round(
                  (endTimeRaw.getTime() - startTime.getTime()) / 60000,
                ),
              )
            : 0;
        ...
        return {
          ...
          // Service info
          serviceId:
            booking.serviceId || booking.services?.[0]?.serviceId || "",
          serviceName: booking.serviceName,
          servicePrice:
            booking.servicePrice ?? booking.price ?? booking.totalAmount,
      ...
      const results = await Promise.all(
        staffIds.map(async (staffId) => {
          try {
            ...
            const blocks = Array.isArray(res?.blocks)
              ? res.blocks
              : res?.data?.blocks || res;
  ...
  const fetchServices = useCallback(async () => {
    try {
      ...
      const serviceRecords = mapApiServicesToRecords(
        servicesResponse?.data ?? [],
      );
      const categoryRecords = mapApiCategoriesToRecords(
        categoriesResponse ?? [],
      );
      ...
      const sanitizeColor = (value: unknown): string | undefined => {
        ...
        if (
          ["auto", "automatic", "inherit", "default", "none"].includes(lowered)
        ) {
          return undefined;
        }
      ...
      const transformedServices: Service[] = serviceRecords.map(
        (rawService) => {
          const service: any = rawService;
          const id =
            typeof service.id === "string" ? service.id : String(service.id);
          ...
          const categoryName =
            ...
            service.categoryName.trim().length > 0
    ...
    try {
      ...
      if (merchantData) {
        ...
        if (merchant.settings) {
          ...
          if (
            merchant.settings.calendarStartHour !== undefined &&
          ...
          if (Array.isArray(merchant.settings.calendarStaffOrder)) {
            const orderedIds = merchant.settings.calendarStaffOrder.filter(
  ...
  const fetchSingleBooking = useCallback(
    async (bookingId: string) => {
      try {
        ...
        if (existingBookingIndex >= 0) {
          actions.dispatch({
            type: "UPDATE_BOOKING",
            payload: { id: bookingId, updates: transformedBooking },
          });
        } else {
          actions.dispatch({
            type: "ADD_BOOKING",
            payload: transformedBooking,
          });
        }
...
export function useTimeGrid() {
  ...
  const timeSlots = useMemo((): TimeSlot[] => {
    ...
    const [businessStartHour, businessStartMinute] = state.businessHours.start
      .split(":")
      .map(Number);
    const [businessEndHour, businessEndMinute] = state.businessHours.end
      .split(":")
      .map(Number);
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      for (let minute = 0; minute < 60; minute += interval) {
        ...
        const businessStartMinutes =
          businessStartHour * 60 + businessStartMinute;
...
export function useBookingOperations() {
  ...
  const updateBookingTime = useCallback(
    async (
      bookingId: string,
      newDate: string,
      newTime: string,
      ...
      if (!existingBooking) {
        const missingMessage =
          "Booking could not be found locally. Please refresh and try again.";
        toast({
          title: "Error",
      ...
      try {
        ...
        const requestedDuration =
          options?.newDuration ?? existingBooking.duration;
        const requestedEndMinutes =
          options?.endMinutes ?? startMinutes + requestedDuration;
        const safeEndMinutes = Math.max(
          requestedEndMinutes,
          startMinutes + Math.max(1, state.timeInterval),
        );
        const updatedDuration =
          options?.newDuration ?? safeEndMinutes - startMinutes;

        const startTimeIso = formatMerchantDateTimeISO(newDate, newTime);
        const endTimeIso = formatMerchantDateTimeISO(
          newDate,
          minutesToTimeString(safeEndMinutes),
        );
        ...
        const staffMember = newStaffId
          ? state.staff.find((s) => s.id === newStaffId)
          : null;
  ...
  const checkTimeConflict = useCallback(
    (
      ...
      return filteredBookings.some((booking) => {
        ...
        // Check for overlap
        return (
          (isAfter(bookingStart, existingStart) &&
            isBefore(bookingStart, existingEnd)) ||
          (isAfter(bookingEnd, existingStart) &&
            isBefore(bookingEnd, existingEnd)) ||
          (isBefore(bookingStart, existingStart) &&
            isAfter(bookingEnd, existingEnd)) ||
  ...
  const navigationLabel = useMemo(() => {
    ...
    switch (state.currentView) {
      case "day":
        ...
        return isToday
          ? "Today"
          : formatInMerchantTime(merchantDate, "EEEE, MMMM d, yyyy");
      case "week":
