...
import {
  ...
  DEFAULT_CALENDAR_STATUS_FILTERS,
} from "./types";
...
const recentStatusUpdates = new Map<
  string,
  { status: BookingStatus; timestamp: number }
...
const recentPreferredStaffSelections = new Map<
  string,
...
function parseStoredStatuses(raw: string | null): BookingStatus[] {
  ...
  try {
    ...
    const normalized = parsed
      .map((value) =>
        typeof value === "string" ? coerceBookingStatus(value) : null,
      )
...
function loadSavedPreferences(): Partial<CalendarState> {
  ...
  try {
    ...
    return {
      ...
      timeInterval: savedTimeInterval
        ? (parseInt(savedTimeInterval) as TimeInterval)
        : 15,
      badgeDisplayMode:
        (localStorage.getItem(STORAGE_KEYS.badgeDisplayMode) as
          | "full"
          | "icon") || "full",
    };
  } catch (error) {
...
const getInitialState = (merchantSettings?: any): CalendarState => {
  ...
  const initialBadgeDisplayMode: "full" | "icon" =
    merchantBadgeMode === "icon" || merchantBadgeMode === "full"
      ? merchantBadgeMode
      : savedPrefs.badgeDisplayMode === "icon" ||
          savedPrefs.badgeDisplayMode === "full"
        ? savedPrefs.badgeDisplayMode
        : "full";
  const initialStaffOrder = Array.isArray(
    freshMerchantSettings?.calendarStaffOrder,
  )
    ? freshMerchantSettings.calendarStaffOrder.filter(
  ...
  return {
    ...
    selectedStatusFilters:
      savedPrefs.selectedStatusFilters || DEFAULT_CALENDAR_STATUS_FILTERS,
    searchQuery: "",
    ...
    showOnlyRosteredStaff:
      freshMerchantSettings?.showOnlyRosteredStaffDefault ?? true,
  ...
  switch (action.type) {
    ...
    case "SET_BOOKINGS": {
      ...
      for (const [
        bookingId,
        selection,
      ] of recentPreferredStaffSelections.entries()) {
      ...
      const incomingIds = new Set(
        normalizedIncoming.map((booking) => booking.id),
      );
      ...
      const preservedLocalBookings = state.bookings.filter((existing) => {
        ...
        const expiry =
          typeof existing.localOnlyExpiresAt === "number"
            ? existing.localOnlyExpiresAt
            : new Date(existing.createdAt).getTime() +
              LOCAL_BOOKING_RETENTION_TIME;
      ...
      const filteredBookings = mergedBookings
        ...
        .map((booking) => {
          ...
          const preferredOverride = recentPreferredStaffSelections.get(
            booking.id,
          );
          ...
          return {
            ...
            ...(preferredOverride
              ? { customerRequestedStaff: preferredOverride.value }
              : null),
          };
        });
    ...
    case "SET_STAFF": {
      ...
      const existingOrder =
        state.staffDisplayOrder.length > 0
          ? state.staffDisplayOrder
          : incomingStaff.map((staff) => staff.id);

      const normalizedOrder = existingOrder.filter((id) =>
        incomingStaff.some((staff) => staff.id === id),
      );
    ...
    case "SET_STAFF_ORDER": {
      ...
      const requestedOrder = action.payload.filter(
        (id, index, self) =>
          typeof id === "string" &&
          id.trim().length > 0 &&
...
function navigateDate(
  ...
  switch (view) {
    case "day":
      return new Date(
        merchantDate.setDate(merchantDate.getDate() + multiplier),
      );
    case "week":
      return new Date(
        merchantDate.setDate(merchantDate.getDate() + 7 * multiplier),
      );
    case "month":
      return new Date(
        merchantDate.setMonth(merchantDate.getMonth() + multiplier),
      );
    default:
      return merchantDate;
  }
...
export function CalendarProvider({ children }: CalendarProviderProps) {
  ...
  const allowUnassignedBookings =
    merchantSettings?.allowUnassignedBookings ?? false;

  const [state, dispatch] = useReducer(
    calendarReducer,
    getInitialState(merchantSettings),
  );
  ...
  const filteredBookings = useMemo(() => {
    ...
    filtered = filtered.filter((booking) => {
      ...
      return (
        bookingDate >= state.dateRange.start &&
        bookingDate <= state.dateRange.end
      );
    });
  ...
  const actions: CalendarActions = useMemo(
    () => ({
      ...
      setTimeInterval: (interval: TimeInterval) =>
        dispatch({ type: "SET_TIME_INTERVAL", payload: interval }),
      ...
      deleteBooking: (id: string) =>
        dispatch({ type: "REMOVE_BOOKING", payload: id }),
      replaceBooking: (oldId: string, newBooking: Booking) =>
        dispatch({ type: "REPLACE_BOOKING", payload: { oldId, newBooking } }),
      setBlocks: (blocks: CalendarBlock[]) =>
        dispatch({ type: "SET_BLOCKS", payload: blocks }),
      addBlock: (block: CalendarBlock) =>
      ...
      setStaffOrder: (order: string[]) =>
        dispatch({ type: "SET_STAFF_ORDER", payload: order }),
      ...
      // Filter actions
      setStaffFilter: (staffIds: string[]) =>
        dispatch({ type: "SET_STAFF_FILTER", payload: staffIds }),
      setServiceFilter: (serviceIds: string[]) =>
        dispatch({ type: "SET_SERVICE_FILTER", payload: serviceIds }),
      setStatusFilter: (statuses: BookingStatus[]) =>
        dispatch({ type: "SET_STATUS_FILTER", payload: statuses }),
      setSearch: (query: string) =>
        dispatch({ type: "SET_SEARCH", payload: query }),
      setBadgeDisplayMode: (mode: "full" | "icon") =>
        dispatch({ type: "SET_BADGE_DISPLAY_MODE", payload: mode }),
      ...
      // Drag actions
      startDrag: (bookingId: string) =>
        dispatch({ type: "START_DRAG", payload: bookingId }),
      ...
      openDetailsSlideOut: (bookingId: string) =>
        dispatch({ type: "OPEN_DETAILS_SLIDEOUT", payload: bookingId }),
  ...
  useEffect(() => {
    ...
    const handleMerchantSettingsUpdate = (e: CustomEvent) => {
      if (e.detail && e.detail.settings) {
        ...
        updateTimeout = setTimeout(() => {
          ...
          const newSettings = {
            ...(enforcedShow !== undefined && {
              showUnassignedColumn: !!enforcedShow,
            }),
            ...
            ...(e.detail.settings.showOnlyRosteredStaffDefault !==
              undefined && {
              showOnlyRosteredStaff:
                e.detail.settings.showOnlyRosteredStaffDefault,
            }),
            ...(e.detail.settings.calendarBadgeDisplayMode !== undefined && {
              badgeDisplayMode:
                e.detail.settings.calendarBadgeDisplayMode === "icon"
                  ? "icon"
                  : "full",
            }),
          };
          ...
          if (Array.isArray(e.detail.settings.calendarStaffOrder)) {
            const incomingOrder = e.detail.settings.calendarStaffOrder.filter(
    ...
    window.addEventListener(
      ...
      handleMerchantSettingsUpdate as EventListener,
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const validStaffIds = state.selectedStaffIds.filter(
        (id, index, self) =>
      ...
      localStorage.setItem(
        STORAGE_KEYS.statusFilters,
        JSON.stringify(state.selectedStatusFilters),
      );
      localStorage.setItem(
        STORAGE_KEYS.staffFilter,
        JSON.stringify(validStaffIds),
      );
      ...
      localStorage.setItem(
        STORAGE_KEYS.timeInterval,
        state.timeInterval.toString(),
      );
      localStorage.setItem(
        STORAGE_KEYS.badgeDisplayMode,
