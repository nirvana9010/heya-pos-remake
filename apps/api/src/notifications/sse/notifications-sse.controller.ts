...
export class NotificationsSseController {
  ...
  async stream(@Query("token") token: string, @Res() res: Response) {
    try {
      ...
      const recentNotifications =
        await this.notificationsService.getNotifications(merchantId, {
          skip: 0,
          take: 10,
        });
  ...
  private setupEventListeners() {
    ...
    this.eventEmitter.on("booking.created", async (event: any) => {
      this.logger.log(
        `Received booking.created event for merchant ${event.merchantId}`,
      );
    ...
    this.eventEmitter.on("booking.updated", async (event: any) => {
      this.logger.log(
        `Received booking.updated event for merchant ${event.merchantId}`,
      );
    ...
    this.eventEmitter.on("notification.created", async (event: any) => {
      this.logger.log(
        `Received notification.created event for merchant ${event.merchantId}`,
      );
  ...
  private async handleBookingEvent(event: any, type: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error handling booking event: ${(error as any).message}`,
      );
    }
  }
  ...
  private async sendToMerchantClients(merchantId: string, data: any) {
    ...
    this.logger.log(
      `Sent ${data.type} to ${merchantClients.length} clients for merchant ${merchantId}`,
    );
  }
