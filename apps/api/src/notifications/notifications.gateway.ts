...
export class NotificationsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  afterInit(server: Server) {
    this.logger.log('🚀 WebSocket Gateway initialized on /notifications namespace');
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      this.logger.log(`✅ Client connected: ${client.id} (User: ${payload.email}, Merchant: ${merchantId}). Total connections: ${this.connectionCount}`);
  ...
  handleDisconnect(client: Socket) {
    ...
    this.logger.log(`👋 Client disconnected: ${client.id} (User: ${client.data.userEmail}). Total connections: ${this.connectionCount}`);
  }
  ...
  handleNotificationEvent(payload: any) {
    ...
    this.logger.debug(`📤 Sent ${channel} to ${merchantSocketCount} clients in merchant ${merchantId}`);
