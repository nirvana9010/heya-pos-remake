...
describeBookingAvailability("BookingAvailabilityService", () => {
  ...
  describe("getAvailableSlots with staff schedules", () => {
    ...
    it("should fall back to business hours when no specific staff is requested", async () => {
      ...
      const morningSlots = result.availableSlots.filter(
        (s) => parseInt(s.time.split(":")[0]) < 12,
      );
      const afternoonSlots = result.availableSlots.filter(
        (s) => parseInt(s.time.split(":")[0]) >= 13,
      );
    ...
    it("should exclude slots that conflict with existing bookings", async () => {
      ...
      const elevenAmSlot = result.availableSlots.find(
        (s) => s.time === "11:00",
      );
    ...
    it("should respect business hours when staff schedule extends beyond them", async () => {
      ...
      const lastSlotHour = parseInt(
        slots[slots.length - 1]?.split(":")[0] || "0",
      );
