import type { Booking, BookingItem, Service } from "../../types";
...
export function createBookingItems(
  ...
  return services.map((service) => ({
    ...
    total:
      service.price * (quantities[service.id] || 1) * (1 + service.taxRate),
...
export function findBookingConflicts(
  ...
  for (const booking of existingBookings) {
    ...
    const hasOverlap =
      (newBooking.startTime >= existingStart &&
        newBooking.startTime < existingEnd) ||
      (newBooking.endTime > existingStart &&
        newBooking.endTime <= existingEnd) ||
      (newBooking.startTime <= existingStart &&
        newBooking.endTime >= existingEnd);
