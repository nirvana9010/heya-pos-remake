...
export class SessionService implements OnModuleDestroy {
  ...
  private readonly SESSION_TIMEOUT =
    parseInt(process.env.SESSION_TIMEOUT_HOURS || "24") * 60 * 60 * 1000; // 24 hours default (was 365 days!)
  ...
  private readonly MAX_SESSIONS_PER_USER = parseInt(
  ...
  createSession(token: string, session: AuthSession): void {
    ...
    if (userSessionCount >= this.MAX_SESSIONS_PER_USER) {
      ...
      if (userSessions.length > 0) {
        ...
        this.logger.warn(
          `User ${session.user.id} session limit reached. Removed oldest session.`,
        );
      }
    }
  ...
  cleanupExpiredSessions(): void {
    ...
    if (cleanedCount > 0) {
      this.logger.log(
        `Cleaned up ${cleanedCount} expired sessions. Active sessions: ${this.sessions.size}`,
      );
    }
  }
  ...
  startCleanupInterval(): void {
    ...
    this.logger.log(
      `Session cleanup interval started (every ${this.CLEANUP_INTERVAL / 60000} minutes)`,
    );
  }
