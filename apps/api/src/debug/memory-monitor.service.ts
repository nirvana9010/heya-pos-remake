...
export class MemoryMonitorService implements OnModuleInit, OnModuleDestroy {
  ...
  private logMemoryUsage() {
    ...
    const uptimeMinutes = ((Date.now() - this.startTime) / 1000 / 60).toFixed(
      1,
    );
    ...
    const heapGrowth =
      this.lastHeapUsed > 0 ? heapUsedMB - this.lastHeapUsed : 0;
    ...
    this.logger[logLevel](
      `Memory Usage${warningMsg}:`,
      JSON.stringify(memoryInfo),
    );

    if (heapGrowth > 50) {
      this.logger.warn(
        `⚠️ Significant heap growth detected: ${heapGrowth.toFixed(2)} MB in last 30 seconds`,
      );
    }

    if (heapUsedMB > 1700) {
      this.logger.error(
        `🚨 CRITICAL: Heap usage approaching limit (${heapUsedMB.toFixed(2)} MB). Memory leak likely!`,
      );
    }
  }
}
