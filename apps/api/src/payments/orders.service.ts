...
export class OrdersService {
  ...
  async updateOrderState(
    ...
    if (order.bookingId) {
      await this.redisService.del(
        RedisService.getOrderByBookingCacheKey(order.bookingId),
      );
    }
  ...
  async recalculateOrderTotals(orderId: string) {
    ...
    console.log(
      `[PERF] Recalc order fetch took ${Date.now() - orderFetchStart}ms`,
    );
    ...
    console.log(
      `[PERF] Parallel updates took ${Date.now() - updateStart}ms (${itemUpdates.length} items, ${modifierUpdates.length} modifiers, 1 order)`,
    );

    console.log(
      `[PERF] RecalculateTotals completed in ${Date.now() - startTime}ms for order ${orderId}`,
    );
  }
  ...
  private async generateOrderNumber(
    ...
    if (retryCount > 10) {
      throw new Error(
        "Unable to generate unique order number after 10 attempts",
      );
    }
    ...
    if (existingOrder) {
      console.log(
        `[generateOrderNumber] Order number ${orderNumber} already exists, retrying...`,
      );
  ...
  async createOrderFromBooking(
    ...
    if (cachedOrderId) {
      console.log(
        `[PERF] Found cached order ID in ${Date.now() - bookingStart}ms`,
      );
      // Verify the order still exists and return it
      try {
        const cachedOrder = await this.findOrderForPayment(
          cachedOrderId,
          merchantId,
        );
        console.log(
          `[PERF] Total createOrderFromBooking took ${Date.now() - bookingStart}ms (cached)`,
        );
        return cachedOrder;
      } catch (e) {
    ...
    if (!actualStaffId) {
      ...
      if (booking.services.length > 0 && booking.services[0].staffId) {
        ...
        if (!defaultStaff) {
          throw new BadRequestException(
            "No active staff found to create order",
          );
        }
    ...
    console.log(
      `[PERF] Order existence check (DB) took ${Date.now() - orderCheckStart}ms`,
    );

    if (existingOrderCheck) {
      console.log(
        "[OrdersService] Order already exists for booking:",
        bookingId,
      );
      ...
      console.log(
        `[PERF] Item count check took ${Date.now() - itemCheckStart}ms`,
      );
      ...
      if (itemCount === 0) {
        ...
        if (orderStateCheck?.state === "DRAFT") {
          ...
          const items = booking.services.map((bs) => ({
            ...
            quantity: 1,
            unitPrice:
              typeof bs.price === "object" && bs.price.toNumber
                ? bs.price.toNumber()
                : Number(bs.price),
            staffId: bs.staffId,
            metadata: {
      ...
      const result = await this.findOrderForPayment(
        existingOrderCheck.id,
        merchantId,
      );
      console.log(
        `[PERF] Find order for payment took ${Date.now() - findOrderStart}ms`,
      );
      console.log(
        `[PERF] Total createOrderFromBooking took ${Date.now() - bookingStart}ms`,
      );
    ...
    const newBookingCacheKey =
      RedisService.getOrderByBookingCacheKey(bookingId);
    ...
    console.log(
      `[PERF] Total createOrderFromBooking took ${Date.now() - bookingStart}ms (new order)`,
    );
    return result;
  }
  ...
  async prepareOrderForPayment(
    ...
    const startTime = Date.now();
    console.log(`[PERF] prepareOrderForPayment - start`, {
      dto,
      userId: user.id,
    });
    ...
    try {
      ...
      parallelFetches.push(
        this.paymentGatewayService
          .getGatewayConfig(user.merchantId)
          .then((config) => {
            console.log(
              `[PERF] Payment gateway fetch took ${Date.now() - gatewayStart}ms`,
            );
            paymentGateway = config;
          })
          .catch((err) => {
            console.error(
              "[PrepareOrder] Failed to fetch payment gateway:",
              err,
            );
            console.log(
              `[PERF] Payment gateway fetch failed after ${Date.now() - gatewayStart}ms`,
            );
      ...
      parallelFetches.push(
        this.prisma.merchant
          ...
          .then((m) => {
            console.log(
              `[PERF] Merchant fetch took ${Date.now() - merchantStart}ms`,
            );
            merchant = m;
          }),
      );
      ...
      if (user.locationId) {
        ...
        parallelFetches.push(
          this.prisma.location
            ...
            .then((l) => {
              console.log(
                `[PERF] Location fetch took ${Date.now() - locationStart}ms`,
              );
              location = l;
            }),
        );
      ...
      if (!dto.orderId) {
        ...
        if (!dto.bookingId) {
          parallelFetches.push(
            this.generateOrderNumber(user.merchantId).then((num) => {
              orderNumber = num;
            }),
          );
        }
      ...
      console.log(
        `[PERF] Pre-fetch completed in ${Date.now() - parallelFetchStart}ms`,
      );
      ...
      if (dto.orderId) {
        ...
        console.log(
          `[PERF] Existing order fetch took ${Date.now() - existingOrderStart}ms`,
        );
        ...
        if (order.customerId !== newCustomerId) {
          console.log(
            `[PrepareOrder] Updating order ${dto.orderId} customerId from ${order.customerId} to ${newCustomerId}`,
          );
        ...
        const existingBookingOrder = await this.prisma.order.findFirst({
          where: {
        ...
        console.log(
          `[PERF] Existing booking order check took ${Date.now() - bookingOrderCheckStart}ms`,
        );

        if (existingBookingOrder) {
          console.log(
            `[PrepareOrder] Found existing order ${existingBookingOrder.id} for booking ${dto.bookingId}`,
          );
        } else {
          console.log(
            `[PrepareOrder] No existing order found for booking ${dto.bookingId}, will create new one`,
          );
        }

        if (existingBookingOrder) {
          console.log(
            `[PrepareOrder] Using existing order ${existingBookingOrder.id} for booking ${dto.bookingId}`,
          );
          ...
          console.log(
            `[PrepareOrder] Creating new order for booking ${dto.bookingId}`,
          );
          ...
          console.log(
            `[PERF] Booking details fetch took ${Date.now() - bookingFetchStart}ms`,
          );
          ...
          order = await this.prisma.$transaction(async (tx) => {
            ...
            console.log(
              `[PERF] Order create took ${Date.now() - orderCreateStart}ms`,
            );
            ...
            if (items.length > 0) {
              ...
              console.log(
                `[PERF] OrderItem createMany took ${Date.now() - itemsCreateStart}ms for ${items.length} items`,
              );
              ...
              console.log(
                `[PERF] OrderItem fetch took ${Date.now() - itemsFetchStart}ms`,
              );
          ...
          console.log(
            `[PERF] Order creation transaction took ${Date.now() - orderCreationTxStart}ms`,
          );

          // Cache the new order ID (OUTSIDE transaction)
          const newBookingCacheKey = RedisService.getOrderByBookingCacheKey(
            booking.id,
          );
          await this.redisService.set(newBookingCacheKey, order.id, 300); // 5 minutes
        }
      ...
      if (dto.items && dto.items.length > 0) {
        ...
        console.log(
          `[PERF] Adding ${dto.items.length} items took ${Date.now() - itemsAddStart}ms`,
        );
      ...
      if (dto.orderModifier) {
        ...
        console.log(
          `[PERF] Order modifier creation took ${Date.now() - modifierStart}ms`,
        );
      }
      ...
      console.log(
        `[PERF] Order totals recalculation took ${Date.now() - recalcStart}ms`,
      );
      ...
      console.log(
        `[PERF] Final order fetch took ${Date.now() - finalOrderFetchStart}ms`,
      );
      ...
      console.log(
        `[PrepareOrder] Order prepared in ${Date.now() - startTime}ms`,
      );
  ...
  private async recalculateOrderTotalsInTransaction(tx: any, orderId: string) {
    ...
    if (itemUpdates.length > 0) {
      await Promise.all(
        itemUpdates.map((update) => tx.orderItem.update(update)),
      );
    }
    ...
    if (modifierUpdates.length > 0) {
      await Promise.all(
        modifierUpdates.map((update) => tx.orderModifier.update(update)),
      );
    }
