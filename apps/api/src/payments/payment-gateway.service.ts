...
export class PaymentGatewayService implements IPaymentGateway, OnModuleInit {
  ...
  async onModuleInit() {
    ...
    console.log(
      "[PaymentGatewayService] Initializing payment gateway on startup:",
      {
        provider,
        merchantId,
    ...
    try {
      ...
      console.log(
        "[PaymentGatewayService] Payment gateway initialized successfully",
      );
    } catch (error) {
      console.error(
        "[PaymentGatewayService] Failed to initialize payment gateway:",
        error,
      );
      ...
      if (process.env.NODE_ENV !== "production" && provider !== "MOCK") {
        console.log(
          "[PaymentGatewayService] Falling back to MOCK provider for development",
        );
        await this.initialize({
          provider: "MOCK",
  ...
  async getGatewayConfig(
    merchantId: string,
  ): Promise<{ provider: string; config: any }> {
