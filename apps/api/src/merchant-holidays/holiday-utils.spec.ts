...
describe("Australian state holiday generator", () => {
  it("generates ACT holidays for 2025 without duplicates", () => {
    ...
    const reconciliation = holidays.find(
      (h) => h.name === "Reconciliation Day",
    );
    expect(reconciliation?.date.toISOString()).toEqual(
  ...
  it("computes Western Australia Day correctly for 2025", () => {
    ...
    expect(kingsBirthday?.date.toISOString()).toEqual(
