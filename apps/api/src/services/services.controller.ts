...
export class ServicesController {
  ...
  @Get(":id")
  @RequirePermissions("service.view")
  findOne(@Param("id") id: string, @CurrentUser() user: any): Promise<Service> {
  ...
  async getCsvMapping(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
  ...
  async previewImport(
    ...
    const options: ImportOptionsDto = {
      ...
      createCategories:
        body.createCategories === "true" || body.createCategories === true,
      skipInvalidRows:
        body.skipInvalidRows === "true" || body.skipInvalidRows === true,
    };
  ...
  async downloadTemplate(@Res() response: Response) {
    ...
    response.header(
      "Content-Disposition",
      'attachment; filename="service-import-template.csv"',
    );
    response.send(csv);
  }
}
