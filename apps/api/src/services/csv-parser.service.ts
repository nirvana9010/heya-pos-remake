...
export class CsvParserService {
  ...
  parseDuration(value: string | number): number {
    ...
    const hoursMinMatch = str.match(
      /^(\d+)\s*h(?:ours?)?\s*(\d+)\s*m(?:ins?)?$/,
    );
    if (hoursMinMatch) {
  ...
  validateRow(
    ...
    const isDelete =
      action === "delete" || action === "remove" || action === "del";
    ...
    if (!row.duration || row.duration === "") {
      if (row.price && merchantSettings?.priceToDurationRatio) {
        warnings.push(
          `Duration will be calculated from price: $${row.price} → ${Math.round(parseFloat(row.price) * merchantSettings.priceToDurationRatio)} minutes`,
        );
      } else {
        errors.push(
          "Duration is required (or enable auto-duration in Import settings)",
        );
      }
    } else {
      try {
  ...
  calculateDurationFromPrice(
    price: number,
    priceToDurationRatio: number = 1.0,
  ): number {
  ...
  transformRow(row: any, merchantSettings?: any): ServiceCsvRowDto {
    ...
    const parseAction = (value: any): ImportAction | undefined => {
      ...
      if (lower === "edit" || lower === "update" || lower === "modify")
        return ImportAction.EDIT;
      if (lower === "delete" || lower === "remove" || lower === "del")
    ...
    if (
      ...
      duration = this.calculateDurationFromPrice(
        price,
        merchantSettings.priceToDurationRatio,
      );
    }
