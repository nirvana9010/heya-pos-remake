...
async function testCheckInLite() {
  ...
  try {
    ...
    if (!checkInLitePackage) {
      console.error(
        "❌ Check-In Lite package not found. Run add-check-in-lite-package.ts first.",
      );
      return;
    }
    ...
    console.log(
      "- Features:",
      JSON.stringify(updatedMerchant.package?.features, null, 2),
    );
    ...
    const features =
      (merchantWithPackage?.package?.features as any)?.enabled || [];
    ...
    console.log(
      "1. Start the merchant app: cd apps/merchant-app && npm run dev",
    );
    console.log(
      "2. Login as Hamilton Beauty Spa (admin@hamiltonbeauty.com / demo123)",
    );
    console.log(
      "3. You should see limited navigation with only Check-In, Customers, and Settings",
    );
