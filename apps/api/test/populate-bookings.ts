...
async function createBooking(token: string, bookingData: any) {
  try {
    ...
    console.log(
      `✓ Created booking: ${response.data.bookingNumber} - ${bookingData.customerName} at ${new Date(bookingData.startTime).toLocaleString()}`,
    );
    return response.data.data || response.data;
  } catch (error: any) {
    console.error(
...
async function populateBookings() {
  ...
  try {
    ...
    console.log(
      `Found ${services?.length || 0} services, ${staff?.length || 0} staff members, ${customers?.length || 0} customers, ${locations?.length || 0} locations\n`,
    );
