...
async function checkTimezoneHandling() {
  try {
    ...
    console.log(
      `Timezone Offset (minutes from UTC): ${new Date().getTimezoneOffset()}`,
    );
    console.log(
    ...
    console.log(
      `Local midnight: ${june1Local.toISOString()} (${june1Local.toString()})`,
    );
    console.log(
      `UTC midnight: ${june1UTC.toISOString()} (${june1UTC.toString()})\n`,
    );
    ...
    console.log(
      `  Start: ${todayStartLocal.toISOString()} (${todayStartLocal.toString()})`,
    );
    console.log(
      `  End: ${todayEndLocal.toISOString()} (${todayEndLocal.toString()})`,
    );
    console.log("UTC Method:");
    console.log(
      `  Start: ${todayStartUTC.toISOString()} (${todayStartUTC.toString()})`,
    );
    console.log(
      `  End: ${todayEndUTC.toISOString()} (${todayEndUTC.toString()})\n`,
    );
    ...
    sampleBookings.forEach((booking) => {
      ...
      console.log(
        `  Date only (ISO): ${booking.startTime.toISOString().split("T")[0]}`,
      );
      console.log(
        `  Date only (Local): ${booking.startTime.toLocaleDateString()}`,
      );
    });
    ...
    const merchantTime = new Date().toLocaleString("en-US", {
