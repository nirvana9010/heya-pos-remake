...
async function checkAuditLogs() {
  try {
    ...
    const auditLogs = await prisma.auditLog.findMany({
      where: {
        OR: [{ entityType: "booking" }, { action: { contains: "booking" } }],
      },
      orderBy: {
        timestamp: "desc",
      },
    ...
    recentLogs.forEach((log) => {
      ...
      if (
        JSON.stringify(log.details).includes("SAMPLE") ||
        JSON.stringify(log.details).includes("HCCHJO")
      ) {
        console.log(
          `  *** Contains SAMPLE or HCCHJO: ${JSON.stringify(log.details)}`,
        );
      }
    });
  } catch (error) {
