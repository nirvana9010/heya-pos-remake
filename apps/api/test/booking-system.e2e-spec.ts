...
describe("Booking System E2E Tests", () => {
  ...
  describe("3. Service Catalog Management", () => {
    ...
    it("should filter services by category", async () => {
      ...
      const hairCategory = categoriesResponse.body.find(
        (c: any) => c.name === "Hair",
      );
  ...
  describe("5. Booking Calendar Functionality", () => {
    it("should check availability for services", async () => {
      ...
      const manicureService = services.find(
        (s) => s.name === "Classic Manicure",
      );
    ...
    it("should prevent double booking", async () => {
      const tomorrow = format(addDays(new Date(), 1), "yyyy-MM-dd");
      const pedicureService = services.find(
        (s) => s.name === "Classic Pedicure",
      );
    ...
    it("should complete booking and update customer stats", async () => {
      ...
      expect(customerAfter.totalSpent).toBeGreaterThan(
        customerBefore.totalSpent,
      );
      expect(customerAfter.loyaltyPoints).toBeGreaterThan(
        customerBefore.loyaltyPoints,
      );
    });
  ...
  describe("7. Performance and Data Integrity", () => {
    it("should handle concurrent bookings", async () => {
      ...
      const bookingPromises = services.body.data.map(
