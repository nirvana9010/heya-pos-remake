...
async function comprehensiveSearch() {
  try {
    ...
    if (bookingWithHCCHJO) {
      ...
      console.log(
        `   Customer: ${bookingWithHCCHJO.customer.firstName} ${bookingWithHCCHJO.customer.lastName}`,
      );
    ...
    customerBookings.forEach((booking) => {
      ...
      console.log(
        `   - ${booking.bookingNumber} on ${date.toDateString()} at ${date.toTimeString()}`,
      );
    ...
    latestBookings.forEach((booking) => {
      console.log(
        `   - ${booking.bookingNumber} for ${booking.customer.firstName} ${booking.customer.lastName}`,
      );
