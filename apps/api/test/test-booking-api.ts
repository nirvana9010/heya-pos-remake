...
async function testBookingAPI() {
  try {
    ...
    const response1 = await axios.get<BookingResponse>(
    ...
    if (response1.data.data.length > 0) {
      console.log("First booking:", {
        ...
        customer:
          response1.data.data[0].customer.firstName +
          " " +
          response1.data.data[0].customer.lastName,
      });
    }
    ...
    const response2 = await axios.get<BookingResponse>(
      "http://localhost:3000/api/bookings",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    ...
    console.log(
      `Found ${response2.data.data.length} bookings (showing upcoming only by default)`,
    );

    // Test 3: Get all bookings including past
    console.log("\nTest 3: Fetching all bookings with includeAll=true");
    const response3 = await axios.get<BookingResponse>(
      "http://localhost:3000/api/bookings",
      {
