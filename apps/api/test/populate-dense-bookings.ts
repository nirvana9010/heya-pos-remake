...
async function populateDenseBookings() {
  ...
  try {
    ...
    console.log(
      `Found ${services.length} services, ${staff.length} staff members, ${customers.length} customers\n`,
    );
    ...
    if (customersToCreate > 0) {
      ...
      console.log(
        `✓ Created ${newCustomers.length} customers. Total: ${customers.length}\n`,
      );
    }
    ...
    console.log(
      "📅 Creating DENSE bookings from 7 days ago to 7 days in the future...\n",
    );
    ...
    for (let d = 0; d <= 14; d++) {
      ...
      for (const slot of timeSlots) {
        ...
        if (
          (slot.hour === 9 && Math.random() < 0.3) ||
          (slot.hour === 18 && slot.minute > 0 && Math.random() < 0.5)
        ) {
          continue;
        }
        ...
        if (Math.random() < bookingDensity) {
          ...
          for (const staffMember of shuffledStaff) {
            ...
            const additionalServices =
              Math.random() < 0.2 ? [getRandomItem(services)] : [];

            const allServices = [
              selectedService,
              ...additionalServices.filter((s) => s.id !== selectedService.id),
            ];
