...
async function testLoyaltyAPI() {
  try {
    ...
    const loginResponse = await axios.post(
      "http://localhost:3000/api/v1/auth/merchant/login",
      {
        username: "HAMILTON",
        password: "demo123",
      },
    ...
    const accessToken =
      loginData.access_token || loginData.accessToken || loginData.token;
    if (!accessToken) {
    ...
    console.log(
      `   Loyalty Points: ${emmaAfter?.loyaltyPoints} (+${Number(emmaAfter?.loyaltyPoints) - Number(emma.loyaltyPoints)})`,
    );
    console.log(
      `   Visit Count: ${emmaAfter?.visitCount} (+${Number(emmaAfter?.visitCount) - Number(emma.visitCount)})`,
    );
    console.log(
      `   Total Spent: $${emmaAfter?.totalSpent} (+$${Number(emmaAfter?.totalSpent) - Number(emma.totalSpent)})`,
    );
    console.log(
      `   Loyalty Visits: ${emmaAfter?.loyaltyVisits} (+${Number(emmaAfter?.loyaltyVisits) - Number(emma.loyaltyVisits)})`,
    );
    ...
    if (error.response?.data?.message === "Unauthorized") {
      console.error(
        "   This might be a permission issue. The endpoint might require specific roles.",
      );
    }
  } finally {
