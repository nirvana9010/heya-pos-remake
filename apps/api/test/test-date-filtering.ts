...
const startInMerchantTz = fromZonedTime(
  dateFromUI + "T00:00:00",
  merchantTimezone,
);
const endInMerchantTz = fromZonedTime(
  dateFromUI + "T23:59:59.999",
  merchantTimezone,
);

console.log(
  `Start of day (merchant TZ -> UTC): ${startInMerchantTz.toISOString()}`,
);
console.log(
  `End of day (merchant TZ -> UTC): ${endInMerchantTz.toISOString()}`,
);
...
console.log(
  `WHERE startTime >= '${dayStart.toISOString()}' AND startTime <= '${dayEnd.toISOString()}'`,
);

console.log("\nMerchant timezone approach would query:");
console.log(
  `WHERE startTime >= '${startInMerchantTz.toISOString()}' AND startTime <= '${endInMerchantTz.toISOString()}'`,
);

console.log("\nUTC approach would query:");
console.log(
  `WHERE startTime >= '${utcStart.toISOString()}' AND startTime <= '${utcEnd.toISOString()}'`,
);
