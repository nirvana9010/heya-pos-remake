...
async function testJune1Api() {
  try {
    ...
    const allBookingsResponse = await api.get<BookingsResponse>(
      "/bookings?includeAll=true",
    );
    ...
    const june1Response = await api.get<BookingsResponse>(
    ...
    if (june1Response.data.data.length > 0) {
      ...
      june1Response.data.data.slice(0, 3).forEach((booking: any) => {
        console.log(
          `   - ${booking.customer.firstName} ${booking.customer.lastName}: ${new Date(booking.startTime).toLocaleString()}`,
        );
      });
    }
    ...
    const todayResponse = await api.get<BookingsResponse>(
      "/bookings?date=2025-06-02",
    );
    ...
    const rangeResponse = await api.get<BookingsResponse>(
      "/bookings?startDate=2025-05-31&endDate=2025-06-02",
    );
