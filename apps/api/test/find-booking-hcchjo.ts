...
async function findBooking() {
  try {
    ...
    if (bookingByNumber) {
      console.log(
        "Found booking by bookingNumber:",
        JSON.stringify(bookingByNumber, null, 2),
      );
    } else {
    ...
    customerBookings.forEach((booking) => {
      console.log(
        `- ID: ${booking.id}, BookingNumber: ${booking.bookingNumber}, Status: ${booking.status}`,
      );
      console.log(
    ...
    if (bookingById) {
      console.log(
        "\nFound booking by ID:",
        JSON.stringify(bookingById, null, 2),
      );
    }
    ...
    allJune13Bookings.forEach((booking) => {
      console.log(
        `- ${booking.bookingNumber}: ${booking.customer.firstName} ${booking.customer.lastName} at ${booking.startTime}`,
      );
    });
    ...
    if (bookingsByPattern.length > 0) {
      ...
      bookingsByPattern.forEach((booking) => {
        console.log(
          `- ${booking.bookingNumber}: ${booking.customer.firstName} ${booking.customer.lastName}`,
        );
      });
    }
  } catch (error) {
