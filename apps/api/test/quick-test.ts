...
async function runQuickTests() {
  ...
  try {
    ...
    console.log(
      `   Status: ${servicesResponse.status} (Expected: 401 - requires auth)`,
    );
    ...
    const customersResponse = await request(app.getHttpServer()).get(
      "/api/customers",
    );
    console.log(
      `   Status: ${customersResponse.status} (Expected: 401 - requires auth)`,
    );
    ...
    const bookingsResponse = await request(app.getHttpServer()).get(
      "/api/bookings",
    );
    console.log(
      `   Status: ${bookingsResponse.status} (Expected: 401 - requires auth)`,
    );
    ...
    console.log(
      `   Status: ${invalidBookingResponse.status} (Expected: 401 - requires auth)`,
    );
