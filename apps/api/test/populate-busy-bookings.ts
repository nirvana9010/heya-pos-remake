...
async function populateBusyBookings() {
  ...
  try {
    ...
    console.log(
      `Found ${services?.length || 0} services, ${staff?.length || 0} staff members, ${customers?.length || 0} customers\n`,
    );

    // Create additional customers if needed (aim for at least 50)
    const targetCustomerCount = 50;
    const customersToCreate = Math.max(
      0,
      targetCustomerCount - customers.length,
    );
    ...
    for (
      ...
      console.log(
        `📅 ${dateKey} (${["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][dayOfWeek]}) - Density: ${Math.round(bookingProbability * 100)}%`,
      );
      ...
      for (const staffMember of staff) {
        ...
        for (const slot of timeSlots) {
          ...
          if (
            (slot.hour === 9 && Math.random() < 0.5) ||
            (slot.hour === 19 && Math.random() < 0.7)
          ) {
            continue;
          }
          ...
          if (Math.random() < bookingProbability) {
            ...
            const serviceCount =
              Math.random() < 0.7 ? 1 : Math.random() < 0.95 ? 2 : 3;
    ...
    console.log(
      "🔄 Creating some overlapping bookings to test conflict handling...\n",
    );
    ...
    console.log(
      `   - Average bookings per day: ${Math.round(totalBookings / Object.keys(bookingsByDay).length)}`,
    );
    console.log(
      `   - Busiest day: ${Object.entries(bookingsByDay)
        .sort((a, b) => b[1] - a[1])[0]
        .join(": ")} bookings`,
    );
