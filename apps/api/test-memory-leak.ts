...
async function stressTestEndpoint(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    if (i % 10 === 0) {
      ...
      console.log(
        `  Iteration ${i}: Memory delta: ${Math.round(totalDelta / 1024 / 1024)}MB`,
      );
    }
  ...
  console.log(`\n[RESULTS] ${endpoint}:`);
  console.log(
    `  Total memory delta: ${Math.round(totalMemoryDelta / 1024 / 1024)}MB`,
  );
  ...
  if (leak > 50 * 1024 * 1024) {
    // 50MB threshold
    console.log(
      `  ⚠️  POSSIBLE MEMORY LEAK: ${Math.round(leak / 1024 / 1024)}MB growth`,
    );
  }
}
