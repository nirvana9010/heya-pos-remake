...
async function testConcurrentBookings(token: string) {
  ...
  const promises = [
    axios.post(`${API_BASE_URL}/v2/bookings`, bookingData, {
      headers: await getHeaders(token),
    }),
    axios.post(`${API_BASE_URL}/v2/bookings`, bookingData, {
      headers: await getHeaders(token),
  ...
  try {
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        ...
        console.log(
          ...
          result.value.data.bookingNumber,
        );
      } else {
        failureCount++;
    ...
    if (successCount === 1 && failureCount === 1) {
      ...
      console.error(
        "❌ Unexpected result - concurrent booking prevention may not be working",
      );
    }
  } catch (error) {
...
async function main() {
  console.log(
    "🚀 Testing Bounded Context Implementation with Transactional Script Pattern\n",
  );
