...
async function seedMoreBookings() {
  ...
  try {
    ...
    console.log(
      `Using ${customers.length} customers, ${staff.length} staff, ${services.length} services\n`,
    );
    ...
    for (let dayOffset = 1; dayOffset <= 7; dayOffset++) {
      ...
      for (let i = 0; i < bookingsPerDay; i++) {
        ...
        const endTime = new Date(
          startTime.getTime() + Number(service.duration) * 60000,
        );
      ...
      console.log(
        `✅ Created ${bookingsPerDay} bookings for ${currentDate.toLocaleDateString()}`,
      );
    }
