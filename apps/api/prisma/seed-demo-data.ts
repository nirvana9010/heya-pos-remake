...
async function seedDemoData() {
  ...
  try {
    ...
    if (!merchant) {
      throw new Error(
        "Hamilton Beauty Spa merchant not found. Please run the main seed first.",
      );
    }
    ...
    console.log(
      `Found ${staff.length} staff members and ${services.length} services\n`,
    );
    ...
    for (let i = 0; i < customersToCreate; i++) {
      const firstName =
        australianFirstNames[
          Math.floor(Math.random() * australianFirstNames.length)
        ];
      const lastName =
        australianLastNames[
          Math.floor(Math.random() * australianLastNames.length)
        ];
    ...
    console.log(
      `✅ Created ${customers.length} new customers (Total: ${allCustomers.length})\n`,
    );

    // 2. Create bookings
    console.log("📅 Creating bookings...");
    await createRealisticBookings(
      merchant.id,
      location.id,
      allCustomers,
      staff,
...
async function createRealisticBookings(
  ...
  const staffSchedules: {
    [key: string]: { [date: string]: Array<{ start: Date; end: Date }> };
  } = {};
  staff.forEach((s) => {
    staffSchedules[s.id] = {};
  });
  ...
  for (let dayOffset = -pastDays; dayOffset <= futureDays; dayOffset++) {
    ...
    for (let i = 0; i < bookingsToday; i++) {
      ...
      staffSchedules[selectedStaff.id][dateKey].push({
        start: startTime,
        end: endTime,
      });
