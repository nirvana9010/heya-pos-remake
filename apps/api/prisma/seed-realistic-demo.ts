...
const CUSTOMER_PROFILES = [
  ...
  },
  {
    ...
    frequency: "occasional",
    visits: 2,
    email: "evelyn.h@email.com",
    phone: "+61 403 111 222",
  },
];
...
async function seedRealisticDemo() {
  try {
    ...
    if (existingCustomers.length < 15) {
      ...
      customers = existingCustomers.map(
        (c, i) =>
          ({
            ...c,
    ...
    await prisma.bookingService.deleteMany({
      where: { booking: { merchantId: merchant.id } },
    });
    ...
    const highPopServices = services.filter((s) =>
      SERVICE_POPULARITY.high.some((name) =>
        s.name.toLowerCase().includes(name.toLowerCase()),
      ),
    );
    const mediumPopServices = services.filter((s) =>
      SERVICE_POPULARITY.medium.some((name) =>
        s.name.toLowerCase().includes(name.toLowerCase()),
      ),
    );
    const lowPopServices = services.filter((s) =>
      SERVICE_POPULARITY.low.some((name) =>
        s.name.toLowerCase().includes(name.toLowerCase()),
      ),
    );
    ...
    for (let d = new Date(startDate); d <= endDate; d = addDays(d, 1)) {
      ...
      switch (dayOfWeek) {
        ...
        case 6:
          baseBookings = 25;
          break; // Saturday - busiest
      }

      // Add some randomness (±20%)
      const dayBookings = Math.floor(
        baseBookings * (0.8 + Math.random() * 0.4),
      );
      ...
      for (let i = 0; i < dayBookings && i < timeSlots.length; i++) {
        ...
        const customerPool =
          Math.random() < 0.5
            ? customers.filter(
                (c) => c.frequency === "vip" || c.frequency === "regular",
              )
            : customers;
        const customer =
          customerPool[Math.floor(Math.random() * customerPool.length)];
        ...
        if (serviceRand < 0.6 && highPopServices.length > 0) {
          service =
            highPopServices[Math.floor(Math.random() * highPopServices.length)];
        } else if (serviceRand < 0.9 && mediumPopServices.length > 0) {
          service =
            mediumPopServices[
              Math.floor(Math.random() * mediumPopServices.length)
            ];
        } else if (lowPopServices.length > 0) {
          service =
            lowPopServices[Math.floor(Math.random() * lowPopServices.length)];
        } else {
        ...
        const booking = await prisma.booking.create({
          data: {
            ...
            depositAmount:
              status === "CANCELLED" ? 0 : Number(service.price) * 0.2,
    ...
    const stats = {
      ...
      regularCustomers: customers.filter((c) => c.frequency === "regular")
        .length,
      occasionalCustomers: customers.filter((c) => c.frequency === "occasional")
        .length,
      totalBookings: totalBookings,
      todayBookings:
        bookingsByDay.get(now.toISOString().split("T")[0])?.length || 0,
    ...
    console.log(
      `     • VIP: ${stats.vipCustomers} (${Math.round((stats.vipCustomers / stats.totalCustomers) * 100)}%)`,
    );
    console.log(
      `     • Regular: ${stats.regularCustomers} (${Math.round((stats.regularCustomers / stats.totalCustomers) * 100)}%)`,
    );
    console.log(
      `     • Occasional: ${stats.occasionalCustomers} (${Math.round((stats.occasionalCustomers / stats.totalCustomers) * 100)}%)`,
    );
...
function generateDayTimeSlots(dayOfWeek: number) {
  ...
  return slots
    ...
    .slice(0, Math.floor(slots.length * 0.8));
}
