...
async function seedDemoFast() {
  ...
  try {
      ...
      DEMO_CUSTOMERS.map(async (cust) => {
        ...
        const existing = await prisma.customer.findFirst({
          where: {
            ...
            OR: [{ email: cust.email }, { mobile: cust.phone }],
          },
        });
      ...
      for (let i = 0; i < numBookings; i++) {
        ...
        const booking = prisma.booking.create({
          data: {
            ...
            bookingNumber:
              `BK${Date.now()}${i}${Math.random().toString(36).substring(2, 4)}`.toUpperCase(),
            status: status,
            startTime: startTime,
    ...
    for (let i = 0; i < bookingPromises.length; i += batchSize) {
      ...
      console.log(
        `Progress: ${Math.min(i + batchSize, bookingPromises.length)}/${bookingPromises.length}`,
      );
    }
    ...
    console.log(`📊 Summary:`);
    console.log(
      `   - Customers: ${totalCustomers} (${DEMO_CUSTOMERS.length} new)`,
    );
