...
async function main() {
  ...
  if (!merchant) {
    throw new Error(
      "Hamilton merchant not found. Please run the main seed first.",
    );
  }
  ...
  const vipCustomers = [
    {
      ...
      tags: ["vip", "premium"],
      totalSpent: 5280.0, // Well over $1000
      visitCount: 24,
      lifetimeVisits: 24,
      loyaltyPoints: 5280,
      loyaltyVisits: 24,
    ...
    },
    {
      ...
      tags: ["vip", "spa-lover"],
      totalSpent: 890.0, // Under $1000 but...
  ...
  for (const customerData of vipCustomers) {
    ...
    console.log(
      `✅ Created VIP customer: ${customer.firstName} ${customer.lastName} - $${customer.totalSpent} spent, ${customer.visitCount} visits`,
    );
  }
  ...
  for (const customerData of regularCustomers) {
    ...
    console.log(
      `✅ Created regular customer: ${customer.firstName} ${customer.lastName}`,
    );
  }
