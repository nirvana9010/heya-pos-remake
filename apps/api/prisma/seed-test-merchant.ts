...
async function seedTestMerchant() {
  ...
  for (let i = 30; i > 0; i -= 3) {
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        bookingNumber:
          `BK${Date.now()}${Math.random().toString(36).substring(2, 4)}`.toUpperCase(),
        status: "COMPLETED",
  ...
  for (const tb of todayBookings) {
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        checkedInAt: ["IN_PROGRESS", "COMPLETED"].includes(tb.status)
          ? bookingTime
          : null,
        completedAt:
          tb.status === "COMPLETED"
            ? new Date(bookingTime.getTime() + tb.service.duration * 60000)
            : null,
        services: {
          create: {
  ...
  for (const fb of futureBookings) {
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        depositAmount: fb.service.requiresDeposit
          ? fb.service.depositAmount || 0
          : 0,
        source: "ONLINE",
        createdById: staffMembers[0].id,
        providerId: staff.id,
  ...
  for (const booking of bookings.filter((b) => b.status === "COMPLETED")) {
    ...
    await prisma.payment.create({
      data: {
        ...
        paymentMethod: ["CASH", "CARD", "CARD_STRIPE"][
          Math.floor(Math.random() * 3)
        ] as "CASH" | "CARD" | "CARD_STRIPE",
  ...
  const loyaltyProgram = await prisma.loyaltyProgram.create({
    data: {
      ...
      expiryDays: 365,
      terms:
        "Points expire after 12 months. Cannot be combined with other offers.",
    },
  });
  ...
  const loyaltyCards = await Promise.all(
    customers.slice(0, 5).map(async (customer, index) => {
      ...
      const spentAmount = customerWithSpent?.totalSpent
        ? Number(customerWithSpent.totalSpent)
        : 0;
  ...
  console.log(
    `Staff PINs: 1234 (Sarah), 2345 (Emma), 3456 (Lisa), 4567 (Jessica), 5678 (Mia)`,
  );
  console.log("\nData created:");
  console.log(
    `- ${await prisma.location.count({ where: { merchantId: merchant.id } })} locations`,
  );
  console.log(
    `- ${await prisma.staff.count({ where: { merchantId: merchant.id } })} staff members`,
  );
  console.log(
    `- ${await prisma.serviceCategory.count({ where: { merchantId: merchant.id } })} service categories`,
  );
  console.log(
    `- ${await prisma.service.count({ where: { merchantId: merchant.id } })} services`,
  );
  console.log(
    `- ${await prisma.customer.count({ where: { merchantId: merchant.id } })} customers`,
  );
  console.log(
    `- ${await prisma.booking.count({ where: { merchantId: merchant.id } })} bookings`,
  );
  console.log(
    `- ${await prisma.invoice.count({ where: { merchantId: merchant.id } })} invoices`,
  );
  console.log(
    `- ${await prisma.loyaltyCard.count({ where: { customerId: { in: customers.map((c) => c.id) } } })} loyalty cards`,
  );
