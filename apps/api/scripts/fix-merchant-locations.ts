...
async function main() {
  ...
  const merchantsWithoutLocations = merchants.filter(
    (m) => m.locations.length === 0,
  );
  console.log(
    `Found ${merchantsWithoutLocations.length} merchants without locations`,
  );
  ...
  const stillWithoutLocations = merchantsAfter.filter(
    (m) => m.locations.length === 0,
  );
  console.log(
