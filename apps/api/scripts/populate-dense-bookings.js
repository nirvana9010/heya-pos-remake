      ...
      for (const slot of timeSlots) {
        ...
        if (Math.random() < bookingDensity) {
          ...
          for (const staffMember of shuffledStaff) {
            ...
            const totalPrice = allServices.reduce((sum, s) => sum + parseFloat(s.price), 0);
            ...
            try {
              ...
              const booking = await prisma.booking.create({
                data: {
                  ...
                  providerId: staffMember.id,  // This is the staff providing the service
                  createdById: staffMember.id, // Using same staff as creator for simplicity
                  bookingNumber: bookingNumber,
                  startTime: startTime,
                  endTime: endTime,
                  status: status,
                  totalAmount: totalPrice,     // Schema uses totalAmount, not totalPrice
              ...
              // Skip payment creation for now as it requires invoices
              // TODO: Create invoices and payments in a separate script

            } catch (error) {
              // Skip on error (likely constraint violation)
              if (error.message.includes('Unique constraint')) {
                // Silently skip duplicate bookings
              } else {
