...
async function main() {
  ...
  if (!merchantId) {
    throw new Error(
      "Merchant ID is required. Pass --merchant <id> or set TRACE_MERCHANT_ID.",
    );
  }

  if (!process.env.SENDGRID_API_KEY) {
    throw new Error(
      "SENDGRID_API_KEY must be set to run the trace against the real email provider.",
    );
  }
  if (!process.env.SENDGRID_FROM_EMAIL) {
    throw new Error(
      "SENDGRID_FROM_EMAIL must be set to run the trace against the real email provider.",
    );
  }
  process.env.USE_MOCKS = "false";

  const customerEmailArg =
    getArg("--customerEmail") || process.env.TRACE_CUSTOMER_EMAIL;
  ...
  try {
    ...
    const settings = merchant.settings
      ? { ...(merchant.settings as Record<string, any>) }
      : {};
    ...
    console.log(
      `🧾 Using service ${service.name}, staff ${staff.firstName} ${staff.lastName || ""}, location ${location.name}`,
    );
    ...
    console.log(
      `✅ Booking created: ${booking.bookingNumber} (${booking.id}) status=${booking.status.value}`,
    );
    ...
    outboxEvents.forEach((evt) => {
      console.log(
        `  - ${evt.eventType} (processedAt=${evt.processedAt?.toISOString() ?? "pending"})`,
      );
      if (evt.lastError) {
