...
async function importData() {
  ...
  try {
    ...
    const metadata = JSON.parse(
      fs.readFileSync(path.join(dataDir, "metadata.json"), "utf-8"),
    );
    ...
    const packages = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Package.json"), "utf-8"),
    );
    ...
    const merchants = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Merchant.json"), "utf-8"),
    );
      ...
      await prisma.merchant.create({
        data: {
          ...
          subscriptionEnds: merchant.subscriptionEnds
            ? new Date(merchant.subscriptionEnds)
            : null,
    ...
    const merchantAuths = JSON.parse(
      fs.readFileSync(path.join(dataDir, "MerchantAuth.json"), "utf-8"),
    );
    ...
    const locations = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Location.json"), "utf-8"),
    );
    ...
    const staff = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Staff.json"), "utf-8"),
    );
    ...
    const staffLocations = JSON.parse(
      fs.readFileSync(path.join(dataDir, "StaffLocation.json"), "utf-8"),
    );
    ...
    const customers = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Customer.json"), "utf-8"),
    );
    ...
    const categories = JSON.parse(
      fs.readFileSync(path.join(dataDir, "ServiceCategory.json"), "utf-8"),
    );
    ...
    const services = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Service.json"), "utf-8"),
    );
      ...
      await prisma.service.create({
        data: {
          ...
          depositAmount: service.depositAmount
            ? parseDecimal(service.depositAmount)
            : null,
    ...
    const bookings = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Booking.json"), "utf-8"),
    );
    ...
    const bookingServices = JSON.parse(
      fs.readFileSync(path.join(dataDir, "BookingService.json"), "utf-8"),
    );
    ...
    const invoices = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Invoice.json"), "utf-8"),
    );
    ...
    const invoiceItems = JSON.parse(
      fs.readFileSync(path.join(dataDir, "InvoiceItem.json"), "utf-8"),
    );
    ...
    const payments = JSON.parse(
      fs.readFileSync(path.join(dataDir, "Payment.json"), "utf-8"),
    );
      ...
      await prisma.payment.create({
        data: {
          ...
          processorResponse: payment.processorResponse
            ? ensureJson(payment.processorResponse)
            : null,
          notes: payment.notes,
    ...
    const refunds = JSON.parse(
      fs.readFileSync(path.join(dataDir, "PaymentRefund.json"), "utf-8"),
    );
    ...
    const programs = JSON.parse(
      fs.readFileSync(path.join(dataDir, "LoyaltyProgram.json"), "utf-8"),
    );
    for (const program of programs) {
      await prisma.loyaltyProgram.create({
        data: {
          ...
          visitRewardValue: program.visitRewardValue
            ? parseDecimal(program.visitRewardValue)
            : null,
          pointsPerVisit: program.pointsPerVisit
            ? parseDecimal(program.pointsPerVisit)
            : null,
          pointsPerDollar: program.pointsPerDollar
            ? parseDecimal(program.pointsPerDollar)
            : null,
    ...
    const tiers = JSON.parse(
      fs.readFileSync(path.join(dataDir, "LoyaltyTier.json"), "utf-8"),
    );
    ...
    const cards = JSON.parse(
      fs.readFileSync(path.join(dataDir, "LoyaltyCard.json"), "utf-8"),
    );
    ...
    const transactions = JSON.parse(
      fs.readFileSync(path.join(dataDir, "LoyaltyTransaction.json"), "utf-8"),
    );
    ...
    const auditLogs = JSON.parse(
      fs.readFileSync(path.join(dataDir, "AuditLog.json"), "utf-8"),
    );
