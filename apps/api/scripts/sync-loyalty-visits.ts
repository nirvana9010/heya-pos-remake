...
async function syncLoyaltyVisits() {
  try {
    ...
    for (const program of visitPrograms) {
      ...
      console.log(
        `Found ${completedBookingsWithoutLoyalty.length} completed bookings without loyalty transactions`,
      );
      ...
      for (const booking of completedBookingsWithoutLoyalty) {
        try {
          ...
          processedCount++;
          console.log(
            `✓ Processed booking ${booking.bookingNumber} for ${customer.firstName} ${customer.lastName || ""}`,
          );
        } catch (error) {
          errorCount++;
          console.error(
            `✗ Error processing booking ${booking.bookingNumber}:`,
            error.message,
          );
        }
      }
      ...
      if (topCustomers.length > 0) {
        ...
        for (const customer of topCustomers) {
          console.log(
            `- ${customer.firstName} ${customer.lastName || ""}: ${customer.loyaltyVisits} visits`,
          );
        }
      }
    }
    ...
    if (
      Array.isArray(customersWithMismatch) &&
      customersWithMismatch.length > 0
    ) {
      console.log(
      ...
      for (const customer of customersWithMismatch as any[]) {
        console.log(
          `- ${customer.firstName} ${customer.lastName || ""}: visitCount=${customer.visitCount}, actual=${customer.actual_bookings}`,
        );
