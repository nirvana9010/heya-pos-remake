  ...
  ({
    ...
    const isFieldValid = (field: keyof typeof customerInfo) => {
      ...
      if (field === "phone") {
        ...
        return (
          phoneRegex.test(value) && value.replace(/[^\d]/g, "").length >= 8
        );
      }
...
export default function BookingPageClient() {
  ...
  const computedMaxAdvanceDays = React.useMemo(() => {
    const bookingAdvanceHours =
      merchantInfo?.settings?.bookingAdvanceHours ?? 168;
  ...
  const calendarToDate = addDays(
    calendarFromDate,
    Math.max(computedMaxAdvanceDays - 1, 0),
  );
  ...
  const disabledDates = React.useCallback(
    (date: Date) => {
      ...
      const todayInTz = TimezoneUtils.startOfDayInTimezone(
        new Date(),
        timezone,
      );
      ...
      if (advanceHours) {
        ...
        const maxDateStartOfDay = TimezoneUtils.startOfDayInTimezone(
          maxDateUTC,
          timezone,
        );
      ...
      for (const service of selectedServicesList) {
        if (service.maxAdvanceBooking) {
          ...
          maxServiceDate.setDate(
            maxServiceDate.getDate() + service.maxAdvanceBooking,
          );
  ...
  useEffect(() => {
    ...
    const fetchStaffForDate = async () => {
      try {
        ...
        console.error(
          "[BookingPageClient] Failed to fetch staff for selected date",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    if (currentStep === confirmationStepId && !confirmationData) {
      console.warn(
        "[BookingPageClient] WARNING: On confirmation step but no confirmationData!",
      );
    }
  ...
  const handleBookingSubmit = async () => {
    ...
    const baseCustomerName =
      formatName(trimmedFirstName, trimmedLastName) || trimmedFirstName;
    const isGuestCustomer = !trimmedEmail && !trimmedPhone;
    const customerNameForSubmission = isGuestCustomer
    ...
    try {
      ...
      setConfirmationData({
        ...
        staffName: selectedStaffMember
          ? cleanStaffName(selectedStaffMember.name)
          : "Any Available",
        date: selectedDate!,
        time: selectedTime!,
        customerName:
          formatName(confirmationFirstName, confirmationLastName) ||
      ...
      console.log(
        "[BookingPageClient] Booking created successfully, moving to confirmation step",
      );
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 4:
        ...
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        const isEmailValid =
          !customerInfo.email || emailRegex.test(customerInfo.email.trim());

        // Validate phone format if phone is provided
        const phoneRegex = /^[\+]?[\d\s\-\(\)]+$/;
        const isPhoneValid =
          !customerInfo.phone ||
          (phoneRegex.test(customerInfo.phone.trim()) &&
            customerInfo.phone.trim().length >= 8);

        const canProceedStep4 =
          hasRequiredFields && isEmailValid && isPhoneValid;
  ...
  const ProgressIndicator = React.memo(function ProgressIndicator() {
    const handleStepClick = (stepId: number) => {
      ...
      if (
        stepId < currentStep ||
        (stepId === currentStep + 1 && canProceed())
      ) {
                ...
                <div key={step.id} className="flex flex-col items-center">
                  <button
                    ...
                    className={cn(
                      ...
                      isActive &&
                        "bg-gradient-to-r from-primary to-secondary shadow-lg scale-125",
                      isCompleted &&
  ...
  const ServiceSelection = () => {
    ...
    const filteredServices = services.filter((service) => {
      ...
      const matchesCategory =
        !selectedCategory || service.categoryName === selectedCategory;
      ...
      selectedServicesList.length > 0
        ? staff.filter(
            (s) =>
              !s.services ||
              s.services.length === 0 ||
              selectedServicesList.some((service) =>
              ...
              >
                <CardHeader>
                  <div className="flex items-center gap-4">
                    ...
                    <motion.div
                      animate={
                        selectedStaff === "" ? { scale: [1, 1.2, 1] } : {}
                      }
                      transition={{ duration: 0.3 }}
                    >
                      <RadioGroupItem
                        id="any-available-radio"
                        value=""
  ...
  const DateTimeSelection = () => {
    ...
    const isClosedByBusinessHours = React.useMemo(() => {
      ...
      const normalizedOpen =
        typeof openValue === "string" ? openValue.toLowerCase() : openValue;
      const normalizedClose =
        typeof closeValue === "string" ? closeValue.toLowerCase() : closeValue;
    ...
    const isShopClosed =
      ...
      (isClosedByBusinessHours ||
        (staff.length === 0 && allStaffRef.current.length > 0));

    // Calculate appointment end time
    const getEndTime = (startTime: string) => {
      const [hours, minutes] = startTime.split(":").map(Number);
      const totalDuration =
        selectedServicesList.reduce((sum, s) => sum + s.duration, 0) || 60;
            ...
            <div className="flex items-center gap-6">
              <div>
                <p className="text-sm text-muted-foreground mb-1">
                  Selected Treatment{selectedServicesList.length > 1 ? "s" : ""}
                </p>
        ...
        >
          <div className="text-center">
            ...
            {(merchantInfo?.settings?.bookingAdvanceHours ||
              merchantInfo?.settings?.minimumBookingNotice) && (
              <div className="text-sm text-muted-foreground mt-2 space-y-1">
                {merchantInfo?.settings?.bookingAdvanceHours && (
                  <p>
                    Bookings can be made up to{" "}
                    {formatAdvanceBookingWindow(
                      Number(merchantInfo.settings.bookingAdvanceHours),
                    )}{" "}
                    in advance
                  </p>
                )}
                {merchantInfo?.settings?.minimumBookingNotice > 0 && (
                  <p>
                    ...
                    {merchantInfo.settings.minimumBookingNotice >= 1440
              ...
              {loadingSlots ? (
                ...
                <div className="max-w-4xl mx-auto space-y-8">
                  {Object.entries(groupedSlots!).map(([period, slots]) => {
                    ...
                    const config =
                      periodConfig[period as keyof typeof periodConfig];
  ...
  const Confirmation = () => {
    ...
    const handleAddToCalendar = () => {
      ...
      const serviceNames = confirmationData.services
        .map((s) => s.name)
        .join(" + ");
      ...
      >
        ...
        >
          ...
          >
            <div className="relative">
              <div
                className={`absolute inset-0 ${merchantInfo?.settings?.autoConfirmBookings === false ? "bg-orange-500/20" : "bg-green-500/20"} rounded-full blur-xl animate-pulse`}
              />
              {merchantInfo?.settings?.autoConfirmBookings === false ? (
            ...
            <CardContent className="space-y-4">
                    ...
                    <div className="border-t pt-2 mt-2">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          ...
                          {merchantInfo?.requireDeposit &&
                            merchantInfo.depositPercentage > 0 && (
                              <p className="text-sm text-muted-foreground mt-1">
                                Deposit paid: $
                                {Math.round(
                                  confirmationData.totalPrice *
                                    (merchantInfo.depositPercentage / 100) *
                                    100,
                                ) / 100}
                              </p>
                            )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {confirmationData.totalDuration} minute session
                  </p>
                </div>
              </div>
        ...
        >
          <Button className="w-full gap-2" onClick={handleAddToCalendar}>
            ...
            <Card className="luxury-card border-0">
              <CardHeader className="text-center pb-8 pt-10">
                <CardTitle className="font-display text-2xl md:text-3xl font-semibold mb-3">
                  ...
                  {currentStep === 5 &&
                    !merchantInfo?.requireDeposit &&
                    `Welcome to ${merchantInfo?.name || merchantFromContext?.name || ""}`}
                  {currentStep === 6 &&
                    `Welcome to ${merchantInfo?.name || merchantFromContext?.name || ""}`}
                </CardTitle>
                <CardDescription className="text-lg text-foreground/60 max-w-2xl mx-auto">
                  {currentStep === 1 &&
                    "Browse our available services to get started"}
                  {currentStep === 2 &&
                    "Choose an available team member for this booking"}
                  {currentStep === 3 &&
                    "Pick a time that works best for your visit"}
                  {currentStep === 4 &&
                    "Just a few details to secure your appointment"}
                  {currentStep === 5 &&
                    merchantInfo?.requireDeposit &&
              ...
              <CardContent>
                    ...
                    {!showCustomerForm ? (
                      <CustomerIdentification
                        onCustomerFound={(customer) => {
                          console.log(
                            "[BookingPageClient] Customer found:",
                            customer,
                          );
                          console.log(
                            "[BookingPageClient] Customer email:",
                            customer.email,
                          );
                          console.log(
                            "[BookingPageClient] Customer data keys:",
                          ...
                          console.log(
                            "[BookingPageClient] Setting customerInfo to:",
                            newCustomerInfo,
                          );
                      ...
                      <div className="flex justify-between mt-12 px-2">
                        ...
                        >
                          <Button
                            onClick={() => {
                              console.log(
                                "[BookingPageClient] Complete Booking clicked",
                                {
                        ...
                        selectedServicesList.length > 0 && (
                          <>
                            ...
                            <PaymentStep
                              ...
                              cancellationHours={
                                merchantInfo?.settings?.cancellationHours
                              }
                            />
                          </>
                        )}
                      {((currentStep === 5 && !merchantInfo?.requireDeposit) ||
                ...
                {(() => {
                  ...
                  const paymentStepExists = steps.some(
                    (s) => s.name === "Payment",
                  );
