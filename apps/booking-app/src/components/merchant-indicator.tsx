...
export function MerchantIndicator() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 bg-background/95 backdrop-blur border rounded-full px-4 py-2 shadow-lg">
      <div
        className={`flex items-center gap-2 text-sm font-medium ${isZen ? "text-green-600" : "text-pink-600"}`}
      >
        {isZen ? (
      ...
      <div className="text-xs text-muted-foreground">({merchantSubdomain})</div>
    </div>
  );
}
