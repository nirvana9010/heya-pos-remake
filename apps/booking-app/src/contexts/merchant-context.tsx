...
export function MerchantProvider({
  ...
  const [merchantSubdomain, setMerchantSubdomain] = useState<string | null>(
  ...
  const detectMerchantSubdomain = async () => {
    try {
      ...
      console.log(
        "[MerchantContext] Fetching merchant subdomain from:",
        apiUrl,
      );
      ...
      if (response.ok) {
        ...
        if (data.subdomain) {
          console.log(
            "[MerchantContext] Detected subdomain from server:",
            data.subdomain,
          );
      ...
      console.error(
        "[MerchantContext] Failed to get subdomain from server:",
        error,
      );
    }
    ...
    const detectionMode =
      process.env.NEXT_PUBLIC_MERCHANT_DETECTION_MODE || "path";
    ...
    if (detectionMode === "subdomain") {
      ...
      if (
        parts.length >= 2 &&
        !["www", "bookings", "app", "api"].includes(parts[0])
      ) {
    ...
    if (!detectedSubdomain && process.env.NODE_ENV === "development") {
      console.log(
        "[MerchantContext] No merchant detected, defaulting to hamilton for development",
      );
    ...
    async function fetchMerchantInfo() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch merchant info: ${response.statusText}`,
          );
        }
        ...
        setError(
          "Unable to load merchant information. Please try again later.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <MerchantContext.Provider
      value={{ merchant, loading, error, merchantSubdomain }}
    >
      {children}
    </MerchantContext.Provider>
  );
