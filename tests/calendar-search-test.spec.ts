...
test.describe("Calendar Customer Search Test", () => {
  test("Test customer search in calendar booking slideout", async ({
    page,
  }) => {
    ...
    await page
      .waitForSelector("[data-calendar-grid]", { timeout: 10000 })
      .catch(() => {
        console.log(
          "⚠️ Calendar grid not found, looking for any calendar view...",
        );
      });
    ...
    for (const selector of slotSelectors) {
      ...
      console.log(
        `📋 Found ${slots.length} elements with selector: ${selector}`,
      );
    ...
    if (!slotClicked) {
      ...
      if (newBookingButton) {
        ...
        console.log(
          "❌ Could not find any time slots or New Booking button to click",
        );
      }
    }
    ...
    await page
      .waitForSelector(
        '[data-slideout], [role="dialog"], .slide-out, div[class*="slide"]',
        {
          timeout: 5000,
        },
      )
    ...
    if (!searchInput) {
      ...
      if (searchEvents.length === 0) {
        ...
        searchEvents.forEach((event, index) => {
          console.log(
            `   ${index + 1}. ${event.request.method} ${event.request.url} - Status: ${event.response?.status}`,
          );
        });
      }
    }
    ...
    console.log(
      `   Customer search API calls: ${monitor.getCustomerSearchEvents().length}`,
    );
  });
});
