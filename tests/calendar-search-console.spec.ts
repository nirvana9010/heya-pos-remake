...
test.describe("Calendar Customer Search with Console Logging", () => {
  test("Capture console logs during customer search", async ({ page }) => {
    ...
    console.log(
      "🧪 Testing customer search on calendar with console logging...",
    );
    ...
    if (calendarArea) {
      ...
      const clickableElements = await calendarArea.$$(
        'div[class*="cursor-pointer"]:not([class*="opacity-50"])',
      );
      console.log(
        `📋 Found ${clickableElements.length} clickable elements in calendar area`,
      );
      ...
      if (clickableElements.length > 0) {
        ...
        slotClicked = true;
        console.log(
          `✅ Clicked element ${index + 1} of ${clickableElements.length}`,
        );
      }
    }
    ...
    try {
      await page.waitForSelector(
        '[data-state="open"], [role="dialog"], .fixed.inset-y-0.right-0',
        {
          timeout: 5000,
        },
      );
    ...
    if (searchEvents.length === 0) {
      ...
      searchEvents.forEach((event, i) => {
        console.log(
          `   ${i + 1}. ${event.request.method} ${event.request.url} - Status: ${event.response?.status}`,
        );
      });
    }
