...
export class NetworkMonitor {
    ...
    this.page.on("response", async (response: Response) => {
      ...
      if (event) {
        try {
          ...
          if (request.url().includes("/customers/search")) {
            console.log(
              `📥 SEARCH RESPONSE: ${response.status()} (${event.response.timing}ms)`,
            );
          ...
          console.log(
            `❌ Response parsing error for ${request.url()}: ${error}`,
          );
        }
      }
    });
  ...
  getSlowRequests(thresholdMs: number = 1000): NetworkEvent[] {
    return this.events.filter(
      (event) => event.response && event.response.timing > thresholdMs,
    );
  }
  ...
  getSearchRequestStats() {
    ...
    const successful = searchEvents.filter(
      (e) => e.response && e.response.status < 400,
    );
    const failed = searchEvents.filter(
      (e) => e.error || (e.response && e.response.status >= 400),
    );
    const average =
      successful.length > 0
        ? successful.reduce((sum, e) => sum + (e.response?.timing || 0), 0) /
          successful.length
        : 0;
  ...
  logSummary() {
    ...
    if (stats.failed > 0) {
      console.log("\n❌ Failed requests:");
      stats.events
        .filter((e) => e.error || (e.response && e.response.status >= 400))
        .forEach((e) => {
          console.log(
            `   ${e.request.method} ${e.request.url} - ${e.error || e.response?.status}`,
          );
        });
    }
  }
