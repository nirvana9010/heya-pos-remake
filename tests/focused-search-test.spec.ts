...
test.describe("Focused Customer Search Test", () => {
  test("Test customer search on booking page with detailed monitoring", async ({
    browser,
  }) => {
    ...
    try {
      ...
      for (const selector of inputSelectors) {
        ...
        console.log(
          `📝 Found ${inputs.length} inputs with selector: ${selector}`,
        );
        ...
        for (let i = 0; i < inputs.length; i++) {
          ...
          console.log(
            `   Input ${i + 1}: placeholder="${placeholder}" name="${name}" id="${id}" visible=${isVisible}`,
          );
          ...
          if (
            ...
            searchInput = input;
            console.log(
              `✅ Selected search input: ${selector} with placeholder="${placeholder}"`,
            );
            break;
          }
        }
      ...
      for (const selector of dropdownSelectors) {
        ...
        if (elements > 0) {
          const visibleElements = await page
            ...
            .count();
          console.log(
            `📋 Found ${elements} elements (${visibleElements} with text) for selector: ${selector}`,
          );
      ...
      console.log(
        "🔄 Testing multiple search queries for intermittent behavior...",
      );
      ...
      for (let i = 0; i < queries.length; i++) {
        ...
        if (newApiCalls === 0) {
          console.log(
            `❌ No API call for "${query}" - potential issue detected!`,
          );
        } else {
      ...
      console.log(
        `⚡ Rapid typing result: ${finalStats.total - totalApiCalls} API calls`,
      );
      ...
      console.log(
        `   Average response time: ${finalStats.averageResponseTime}ms`,
      );
      ...
      if (finalStats.total < queries.length) {
        ...
        console.log(
          `   Expected ${queries.length} API calls, got ${finalStats.total}`,
        );
      }
      ...
      const searchEvents = allEvents.filter((e) =>
        e.request.url.includes("/customers/search"),
      );
      ...
      if (searchEvents.length > 0) {
        ...
        searchEvents.forEach((event, i) => {
          console.log(
            `   ${i + 1}. ${event.request.method} ${event.request.url}`,
          );
          if (event.response) {
            console.log(
              `      Status: ${event.response.status}, Time: ${event.response.timing}ms`,
            );
            if (event.response.body?.data) {
              console.log(
                `      Results: ${event.response.body.data.length} customers`,
              );
            }
          } else if (event.error) {
