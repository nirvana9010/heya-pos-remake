...
test.describe("Final Slideout Test", () => {
  test("Test customer search in calendar slideout with proper detection", async ({
    page,
  }) => {
    ...
    if ((await searchInput.count()) > 0 && (await searchInput.isVisible())) {
      ...
      if (await dropdown.isVisible()) {
        ...
        for (let i = 0; i < Math.min(3, resultButtons.length); i++) {
          ...
          console.log(
            `  Result ${i + 1}: ${text?.replace(/\s+/g, " ").trim()}`,
          );
        }
      } else {
    ...
    if (searchLogs.length === 0) {
      ...
      console.log(
        "  This means the console.log statements in the component are not executing",
      );
    } else {
