...
test("API Search Direct Test", async ({ page }) => {
  ...
  // Search for "Lukas"
  const lukasResponse = await page.request.get(
    "http://localhost:3000/api/v1/customers/search?q=Lukas",
    {
      headers: {
  ...
  if (lukasData.data.length > 0) {
    console.log(
      `  - ${lukasData.data[0].firstName} ${lukasData.data[0].lastName}`,
    );
  }

  // Search for "Test"
  const testResponse = await page.request.get(
    "http://localhost:3000/api/v1/customers/search?q=Test",
    {
      headers: {
        Cookie: `auth-token=${authCookie.value}`,
      },
  ...
  for (let i = 0; i < Math.min(3, testData.data.length); i++) {
    console.log(
      `  - ${testData.data[i].firstName} ${testData.data[i].lastName}`,
    );
  }
