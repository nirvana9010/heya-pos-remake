...
export class CalendarPage {
  ...
  constructor(page: Page) {
    ...
    this.calendarGrid = page
      .locator('[data-testid="calendar-grid"], .calendar-grid')
      .first();
  ...
  async waitForCalendarLoad() {
    ...
    await this.page.waitForSelector(
      '[data-testid="calendar-grid"], .calendar-grid, .calendar-container',
      {
        timeout: 10000,
      },
    );
  ...
  async clickEmptyTimeSlot(options?: { hour?: number; date?: string }) {
    ...
    if (!timeSlotClicked) {
      try {
        ...
        const calendarArea = this.page
          .locator(
            '.calendar-grid, .calendar-container, [data-testid="calendar"]',
          )
          .first();
        ...
        throw new Error(
          `Failed to click any time slot. Tried selectors: ${selectors.join(", ")}`,
        );
      }
    }
  ...
  async getTodayEmptySlots() {
    ...
    const slots = await this.page
      .locator(".calendar-slot:not(:has(.booking))")
      .all();
    return slots.length;
  }
  ...
  async verifyCalendarVisible() {
    await expect(
      this.calendarGrid.or(this.page.locator(".calendar-container")),
    ).toBeVisible();
  }
}
