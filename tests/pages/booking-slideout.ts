...
export class BookingSlideout {
  ...
  constructor(page: Page) {
    this.page = page;
    this.slideout = page
      .locator('[data-testid="booking-slideout"], .slideout, .booking-form')
      .first();
    this.customerSearchInput = page.locator(
      'input[placeholder*="Search"], input[placeholder*="customer"], [data-testid="customer-search"]',
    );
    this.searchDropdown = page
      .locator('.absolute.z-50, [data-testid="search-dropdown"]')
      .first();
    this.searchResults = page.locator(
      '.absolute.z-50 button, [data-testid="search-result"]',
    );
    this.loadingIndicator = page.locator(
      '.animate-spin, [data-testid="loading"]',
    );
    this.nextButton = page.locator(
      'button:has-text("Next"), [data-testid="next-button"]',
    );
    this.closeButton = page.locator(
      'button:has-text("Close"), [data-testid="close-button"], button:has-text("×")',
    );
  }
  ...
  async selectCustomer(index: number = 0) {
    ...
    if (index >= resultElements.length) {
      throw new Error(
        `Cannot select customer at index ${index}. Only ${resultElements.length} results available.`,
      );
    }
