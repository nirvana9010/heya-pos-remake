...
test.describe("Customer Search Issue Investigation", () => {
  ...
  test("Basic customer search flow - should work consistently", async ({
    page,
  }) => {
  ...
  test("Search dropdown visibility - should show and hide correctly", async ({
    page,
  }) => {
  ...
  test("Multiple search cycles - reproduce intermittent issue", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < searches.length; i++) {
      ...
      try {
        ...
        if (results.length > 0) {
          successfulSearches++;
          console.log(
            `✅ Search ${i + 1} successful - ${results.length} results`,
          );
        } else {
          failedSearches++;
    ...
    if (failedSearches === searches.length) {
      throw new Error(
        "All searches failed - this indicates the search issue is occurring",
      );
    }
  });

  test("Rapid typing simulation - test debouncing behavior", async ({
    page,
  }) => {
    ...
    const searchStats = networkMonitor.getSearchRequestStats();
    console.log(
      `🌐 Rapid typing resulted in ${searchStats.total} API requests`,
    );
  ...
  test("Network timing analysis - detect slow responses", async ({ page }) => {
    ...
    for (const query of queries) {
      ...
      console.log(
        `⏱️  Search "${query}" took ${endTime - startTime}ms (including debounce)`,
      );
    ...
    console.log(
      `📊 Average API response time: ${searchStats.averageResponseTime}ms`,
    );
