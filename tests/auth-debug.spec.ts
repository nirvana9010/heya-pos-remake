...
test.describe("Authentication Debug", () => {
  test("Investigate login process step by step", async ({ page }) => {
    ...
    await page.screenshot({
      path: "auth-debug-1-login-page.png",
      fullPage: true,
    });
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      console.log(
        `   ${i + 1}. type="${type}" name="${name}" placeholder="${placeholder}" id="${id}"`,
      );
    }
    ...
    if (await quickLoginButton.isVisible({ timeout: 1000 })) {
      ...
      await page.screenshot({
        path: "auth-debug-2-form-filled.png",
        fullPage: true,
      });
    ...
    await page.screenshot({
      path: "auth-debug-3-after-submit.png",
      fullPage: true,
    });

    // Step 6: Check for errors or success indicators
    const errorElements = await page
      .locator('.error, .alert-error, [class*="error"], .text-red')
      .all();
    ...
    if (page.url().includes("/login")) {
      ...
      console.log(
        "✅ Redirected away from login page - authentication may have succeeded",
      );
    }
