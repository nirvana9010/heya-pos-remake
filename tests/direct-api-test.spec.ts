...
test.describe("Direct API Test", () => {
  test("Test customer search API directly from browser console", async ({
    page,
  }) => {
    ...
    const consoleResult = await page.evaluate(() => {
      ...
      const testSearch = async () => {
        try {
          ...
          console.log(
            "🔍 CustomerSearchInput: apiClient available?",
            !!apiClientModule,
          );
          ...
          console.log(
            "🔍 CustomerSearchInput: apiClient.searchCustomers available?",
            !!apiClientModule?.searchCustomers,
          );

          const response = await apiClientModule.searchCustomers("test");
          console.log(
            "✅ CustomerSearchInput: API call succeeded, response:",
            response,
          );
          ...
          console.error(
            "❌ CustomerSearchInput: Customer search failed:",
            error,
          );
