...
test.describe("Search Debug", () => {
  test("Debug search state issue", async ({ page }) => {
    ...
    page.on("response", async (response) => {
      if (response.url().includes("/customers/search")) {
        ...
        try {
          ...
          console.log(
            `🌐 API Response for "${query}": ${data.data.length} results`,
          );
          if (data.data.length > 0) {
            console.log(
              `  First result: ${data.data[0].firstName} ${data.data[0].lastName}`,
            );
          }
        } catch (e) {
    ...
    if (lukasStillVisible && searchRequests.some((r) => r.query === "Test")) {
      console.log(
        "  ❌ CONFIRMED: Lukas still showing after searching for Test",
      );
