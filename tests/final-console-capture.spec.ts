...
test.describe("Final Console Capture", () => {
  test("Capture console logs showing why search fails", async ({ page }) => {
    ...
    const newBookingButton = page
      .locator('button:has-text("New Booking")')
      .first();
    ...
    if (await newBookingButton.isVisible()) {
      ...
      const slideoutInputs = await page
        .locator('.fixed.inset-y-0.right-0 input, [data-state="open"] input')
        .all();
      ...
      for (let i = 0; i < slideoutInputs.length; i++) {
        ...
        if (
          ...
          const searchRequest = await page
            .waitForRequest((req) => req.url().includes("/customers"), {
              timeout: 3000,
            })
            .catch(() => null);
