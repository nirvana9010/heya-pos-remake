...
test.describe("Visual Search Test", () => {
  test("Visual debugging of customer search", async ({ page }) => {
    ...
    const pageContent = await page.evaluate(() => {
      ...
      const clickables = Array.from(document.querySelectorAll("*")).filter(
        (el) => {
    ...
    const timeSlots = await page
      .locator('div[class*="cursor-pointer"][class*="hover\\:bg-gray"]')
      .all();
    ...
    let slideoutVisible = await page
      .locator(".fixed.inset-y-0.right-0")
      .isVisible()
      .catch(() => false);
    ...
    await page.screenshot({
      path: "screenshots/calendar-click-attempt.png",
      fullPage: true,
    });
    ...
    if (slideoutVisible) {
      ...
      const customerInput = await page
      ...
      if (await customerInput.isVisible()) {
        ...
        await page.screenshot({
          path: "screenshots/search-results-visible.png",
          fullPage: true,
        });
