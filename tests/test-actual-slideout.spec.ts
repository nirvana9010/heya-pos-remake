...
test.describe("Test Actual Calendar Slideout", () => {
  test("Test customer search in the actual calendar slideout", async ({
    page,
  }) => {
    ...
    const emptySlots = page.locator(
      "div.cursor-pointer.h-\\[60px\\]:not(:has(*))",
    );
    ...
    if (clicked) {
      ...
      let slideout = null;
      try {
        slideout = await page.waitForSelector(
          '.fixed.inset-y-0.right-0, [data-state="open"], [role="dialog"]',
          {
            timeout: 5000,
            state: "visible",
          },
      ...
      if (slideout) {
        ...
        for (const step of steps) {
          const isActive =
            (await page
              .locator(`button:has-text("${step}").bg-teal-100`)
              .count()) > 0;
          if (isActive) {
            currentStep = step;
        ...
        if (currentStep !== "Customer") {
          ...
          const customerTab = page
            .locator('button:has-text("Customer")')
            .first();
        ...
        if ((await searchInput.count()) > 0) {
          ...
          const allInputs = await page
            .locator(".fixed.inset-y-0.right-0 input")
            .all();
          ...
          for (let i = 0; i < allInputs.length; i++) {
            ...
            console.log(
              `  Input ${i + 1}: type="${type}" placeholder="${placeholder}"`,
            );
          }
        }
      } else {
