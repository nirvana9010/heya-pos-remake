  ...
  test("Investigate calendar page structure", async ({ browser }) => {
    ...
    try {
      ...
      const calendarElements = await page
        .locator('[class*="calendar" i], [id*="calendar" i]')
        .all();
      console.log(
        `🔍 Found ${calendarElements.length} elements with "calendar" in class/id`,
      );
      ...
      const loading = await page
        .locator('[class*="loading" i], [data-loading="true"]')
        .count();
      console.log(`⏳ Loading elements found: ${loading}`);

      // Check for error messages
      const errors = await page
        .locator('[class*="error" i], .alert-error, .error-message')
        .count();
