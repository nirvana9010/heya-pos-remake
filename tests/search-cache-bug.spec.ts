...
test.describe("Search Cache Bug", () => {
  test("Reproduce search cache issue - search Lukas, clear, search Test", async ({
    page,
  }) => {
    ...
    page.on("console", async (msg) => {
      ...
      if (
        text.includes("CustomerSearchInput") ||
        text.includes("🔍") ||
        text.includes("❌") ||
        text.includes("✅") ||
        text.includes("⚠️")
    ...
    const emptySlot = page
      .locator("div.cursor-pointer.h-\\[60px\\]:not(:has(*))")
      .first();
    ...
    const serviceButton = page
      .locator('button:has(h4:text("Test new service"))')
      .first();
    ...
    const dropdown = page
      .locator('[role="listbox"], .absolute.z-50')
      .filter({ has: page.locator("text=/Lukas/i") });
    ...
    const customerResults = await page
      .locator('button[role="option"], button:has(.flex-col)')
      .filter({ hasText: /@|\+/ })
      .all();
    ...
    // Get customer results specifically
    const testCustomerResults = await page
      .locator('button[role="option"], button:has(.flex-col)')
      .filter({ hasText: /@|\+/ })
      .all();
    console.log(
      `📊 Found ${testCustomerResults.length} customer results for "Test"`,
    );
    ...
    const stillHasLukas =
      (await page.locator('button:has-text("Lukas Nguyen")').count()) > 0;
    if (stillHasLukas) {
      console.log(
    ...
    console.log(
      `Actual: ${stillHasLukas ? 'Still showing "Lukas" results' : "Showing correct results"}`,
    );
