...
test.describe("Proper Slideout Search Test", () => {
  test("Open booking slideout and test customer search", async ({ page }) => {
    ...
    const timeSlot = await page
      .locator("div.cursor-pointer.h-\\[60px\\]")
      .first();
    ...
    if (await timeSlot.isVisible()) {
      ...
      const slideout = await page
        .waitForSelector('[data-state="open"], .fixed.inset-y-0.right-0', {
          timeout: 5000,
        })
        .catch(() => null);
        ...
        if (correctInput) {
          ...
          if (await dropdown.isVisible()) {
            ...
            const results = await page
              .locator('.z-50 button[type="button"]')
              .count();
          ...
          await page.screenshot({
            path: "screenshots/customer-search-state.png",
            fullPage: true,
          });
